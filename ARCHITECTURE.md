...
Search uses the current authorized Document generation. PostgreSQL holds bounded chunk text and provenance; OpenSearch holds BM25/vector projection data. The embedding model is a search configuration generation stored in PostgreSQL (one PRESENT per deployment, seeded from deployment configuration on first start); each generation owns one index named by the generation, and readiness is recorded per index, so a new model can be built beside the one being served. Query filters narrow an already authorized scope and never create authority. Search results remain source passages; answer generation belongs to Chat.
