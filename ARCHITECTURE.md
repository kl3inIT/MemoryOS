...
Google setup at `/admin/sources/new/google-drive` presents Credential → Connector. The owner selects a reusable credential or creates one through owner-supplied Web OAuth client JSON and separate Google consent. The callback selects the credential; it does not create a Source. Connector submission atomically creates a Source with its own name, validated explicit links and initial sync intent, without another consent. The OAuth modal keeps app/redirect configuration under collapsed Setup instructions. Detail manages revision-aware selection, synchronization and shared-credential reconnect/revoke; conflicts require reloading current authority. No account browser exists. Google remains RESTRICTED: neither source management nor successful extraction grants document-read access.
...
The worker composes a two-thread db-scheduler control plane over PostgreSQL. Topology reconciliation maintains INGESTION, SOURCE_SYNC and CLEANUP Redis Streams/groups. Bounded recurring tasks enqueue due Google sources, relay each workload, cancel inactive-Tenant work, and clean abandoned browser uploads, tracked raw writes and unreferenced extraction artifacts. `scheduled_tasks` owns control-task heartbeat/recovery, not business authority. Redis, datasource, scheduler and private object-storage sentinel readiness are required for the worker; API readiness remains independent of Redis.
...
Each Google credential pins its provider subject and may serve multiple Sources in the same Tenant. Each Source owns a separate Connector/Pair, scope, synchronization progress and item/Document identity. SPECIFIC accepts at most 20 nonoverlapping explicit file/folder roots. GENERAL resolves and validates the credential's actual My Drive root, stores it internally, and exposes an empty user-selected roots list. Every SOURCE_SYNC reconciles the persisted roots and folder descendants through the same durable frontier, paging, snapshot and indexing pipeline. General does not enumerate Shared with me, Shared Drives, other users' drives or an account-wide Changes feed; Specific can select an explicit file/folder inside a Shared Drive. Whole-drive links, shortcuts and overlapping Specific selections fail closed.

General revalidates the stored My Drive root against the current provider session before traversal and again before pruning. A missing, changed, trashed, shared-drive or otherwise unverifiable root fails the run rather than proving an empty scope. An incomplete generation releases successfully adopted, confirmed inputs for indexing but does not prune unseen items. Changing mode or roots uses the same scope revision and fences obsolete acquisition/publication; only complete reconciliation under the new authority can prune excluded content. Owners configure the per-Source interval with an independent schedule revision; changing the interval does not fence work or invalidate Documents, and changing scope does not overwrite the interval. Minute-level due checks and manual sync remain unchanged.
...
Shared reconnect, revoke and accepted authentication failure fence sync/index work and retrieval mappings for every attached Source in one transaction. Authority-sensitive locks follow Tenant → credential → Source order, with attached Sources ordered deterministically. Source creation validates provider metadata outside SQL transactions and rechecks owner and credential authority before atomic persistence. Source cleanup retains the reusable credential; explicit credential deletion requires no attachments and the current authority revision.
