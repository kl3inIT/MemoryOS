    ...
    Result validate(ActorId actor, UUID id) {
        ...
        try (var resolved = models.forValidation(actor, id)) {
            ...
            try {
                ...
                var options = binding.service().convertOptions(new LlmOptions().withMaxTokens(Math.min(32, binding.maxOutputTokens())));
                var prompt = binding.finalRequest().apply(new Prompt(List.of(new UserMessage("Reply OK.")), options));
