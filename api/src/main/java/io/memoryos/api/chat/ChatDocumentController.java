...
@Tag(name = "Chat")
@ApiResponse(responseCode = "400", description = "Invalid passage window")
@ApiResponse(responseCode = "403", description = "Tenant membership or CSRF requirement not met")
@ApiResponse(responseCode = "404", description = "Document generation not readable")
...
class ChatDocumentController {
    ...
    @ApiResponse(responseCode = "416", description = "Requested byte range starts beyond the original", content = @Content)
    ...
    ResponseEntity<DocumentSpreadsheetResponse> spreadsheet(@CurrentActor IdentityContext identity,
            @PathVariable UUID documentId, @RequestParam UUID generation) {
    ...
    ResponseEntity<SearchDocumentResponse> passages(@CurrentActor IdentityContext identity,
