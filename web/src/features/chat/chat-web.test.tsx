...
it("keeps Web off when no search connection is configured", async () => {
  ...
  const change = vi.fn();
  render(<ChatActionsMenu value="off" disabled={false} onChange={change} />);
  await userEvent.click(screen.getByRole("button", { name: "Actions" }));
  await userEvent.click(screen.getByRole("button", { name: "Web options" }));
  expect(screen.getByRole("option", { name: "Use Web automatically" })).toHaveAttribute(
    "aria-disabled",
    "true",
  );
...
it("uses adapter capabilities for required mode, never model-name guesses", async () => {
  const change = vi.fn();
  render(<ChatActionsMenu value="off" modelId="b" disabled={false} onChange={change} />);
  await userEvent.click(screen.getByRole("button", { name: "Actions" }));
  await userEvent.click(screen.getByRole("button", { name: "Web options" }));
  expect(screen.getByRole("option", { name: "Require Web search" })).toHaveAttribute(
    "aria-disabled",
    "true",
  );
  await userEvent.click(screen.getByRole("option", { name: "Use Web automatically" }));
...
it("selects the single Web action for this turn and keeps options accessible", async () => {
  const change = vi.fn();
  render(<ChatActionsMenu value="off" disabled={false} onChange={change} />);
  await userEvent.click(screen.getByRole("button", { name: "Actions" }));
  expect(screen.getAllByRole("option")).toHaveLength(1);
  expect(screen.getByRole("button", { name: "Web options" })).toBeEnabled();
  await userEvent.click(screen.getByRole("option", { name: "Web search" }));
