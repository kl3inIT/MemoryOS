...
export default function ShikiHighlighter({ code, language }: { code: string; language: string }) {
  ...
  return (
    <div className="aui-shiki-base [&_pre]:overflow-x-auto [&_pre]:rounded-b-xl [&_pre]:border [&_pre]:border-t-0 [&_pre]:border-border/50 [&_pre]:bg-muted/30! [&_pre]:p-3.5 [&_pre]:text-code [&_pre]:leading-relaxed [&_.line]:px-0!">
      {highlighted ?? (
        <pre>
          <code>{code}</code>
        </pre>
