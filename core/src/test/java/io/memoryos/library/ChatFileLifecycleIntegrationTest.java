...
class ChatFileLifecycleIntegrationTest {
    ...
    void uploadIdentityIsOwnerPrivateAndFinalizeQueuesExactlyOnce() {
        ...
        assertThrows(LibraryException.class, () -> files.get(other, first.file().id()));
        assertThrows(LibraryException.class, () -> files.finalizeUpload(other, first.file().id()));
        assertThrows(LibraryException.class, () -> files.initiate(owner, input(input.requestId(), 5)));
    ...
    void mismatchedMetadataAndRevocationDuringProviderIoNeverAdopt() {
        ...
        assertThrows(LibraryException.class, () -> files.finalizeUpload(owner, id));
    ...
    void controlPlaneExpiresAbandonedUploadAndRetryRequiresNewUpload() {
        ...
        assertThrows(LibraryException.class, () -> files.retry(owner, id));
        assertEquals(0, count("chat_file_work"));
    }
    ...
    void plaintextRangesUseUnicodeCodePointsAndNeverBypassOwnerOrDeletion() {
        var id = finalized();
        assertThrows(LibraryException.class, () -> files.read(owner, id, 0, 10));
        var delivery = dispatch();
        var claim = work.claim(tenant, delivery.operationId().value(), delivery.deliveryId()).orElseThrow();
        ...
        assertThrows(LibraryException.class, () -> files.read(owner, id, 0, 16001));
        assertThrows(LibraryException.class, () -> files.read(other, id, 0, 10));
        assertThrows(LibraryException.class, () -> files.read(owner, new TenantId(UUID.randomUUID()), id, 0, 10));
        files.delete(owner, id);
        assertThrows(LibraryException.class, () -> files.read(owner, id, 0, 10));
    }
    ...
    void originalContentClosesOnRevocationDuringStorageIoAndNeverOpensForAnotherOwner() {
        ...
        var claim = work.claim(tenant, delivery.operationId().value(), delivery.deliveryId()).orElseThrow();
        assertTrue(work.complete(claim, content()));
        assertThrows(LibraryException.class, () -> fileContent.open(other, id));
        ...
        assertThrows(LibraryException.class, () -> fileContent.open(owner, id));
        verify(stream).close();
        assertThrows(LibraryException.class, () -> files.read(owner, id, 0, 10));
    }
    ...
    void uploadedImageRendersOneThumbnailThatIsReusedAndReleasedWithTheUpload() throws Exception {
        ...
        assertEquals(2, count("stored_objects"));
        assertThrows(LibraryException.class, () -> fileContent.thumbnail(other, id));
    ...
    void anUploadThatIsNotAnImageHasNoThumbnail() {
        ...
        assertEquals("Only image files have a thumbnail",
                assertThrows(LibraryException.class, () -> fileContent.thumbnail(owner, id)).getMessage());
        verify(storage, never()).open(any());
    }
