...
public record ModelRequestPolicy(TokenCountEstimator tokens, ToIntFunction<Prompt> framing,
    ...
    /** Admission estimate for one image input, not reported provider usage. The native response remains the usage ledger. */
    ...
    public void requireFits(List<Message> messages, int inputBudget) {
        if (framing.applyAsInt(new Prompt(messages)) > inputBudget)
            throw AiException.invalid("The current prompt exceeds the configured context limit.");
    }
