...
public final class ProviderCredentials {
    ...
    public @Nullable String update(UUID tenant, UUID provider, @Nullable String previous, Change change) {
        if (change == null || change.action() == null) throw AiException.invalid("Credential action is required.");
        if (change.action() != Action.REPLACE && change.value() != null)
            throw AiException.invalid("Only credential replacement accepts a value.");
    ...
    public String resolve(UUID tenant, UUID provider, @Nullable String stored) {
        ...
        try {
            ...
            if (!plain.startsWith(prefix)) throw AiException.providerUnavailable();
            return plain.substring(prefix.length());
        } catch (RuntimeException invalid) { throw AiException.providerUnavailable(); }
    }

    private AesGcmBytesEncryptor requireEncryptor() {
        if (encryptor == null) throw AiException.providerUnavailable();
        return encryptor;
    }
}
