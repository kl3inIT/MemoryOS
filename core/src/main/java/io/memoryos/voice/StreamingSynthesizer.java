...
public final class StreamingSynthesizer implements AutoCloseable {
    ...
    public synchronized void append(String text) {
        ...
        if (text.length() > VoiceSynthesisService.MAX_SEGMENT_LENGTH)
            throw VoiceException.invalid("A part to read aloud must contain at most 4096 characters.");
        if (length + text.length() > VoiceSynthesisService.MAX_TEXT_LENGTH)
            throw VoiceException.invalid("Text to read aloud must contain at most 32000 characters.");
    ...
    private void speak(String part) {
        ...
        try {
            ...
            if (!isClosed()) finished.completeExceptionally(VoiceException.providerUnavailable());
        } finally {
            synchronized (this) {
                current = null;
            }
