...
public class TranscriptSummarizer {
    ...
    private final ChatModelExecutor executor;
    private final @Nullable AiUsageRecorder usage;

    public TranscriptSummarizer(ChatModelResolver models, ChatModelExecutor executor, ObjectProvider<AiUsageRecorder> usage) {
        this.models = models;
    ...
    public TranscriptSummary summarize(ActorId actor, UUID tenant, Subject subject, List<Line> lines) {
        if (lines.isEmpty()) throw ChatException.invalid("A transcript is required.");
        try (var selected = models.resolveFlow(actor, ModelFlow.MEETING_MINUTES)) {
            var summary = executor.generateObject(selected.binding(), instructions(subject), transcript(subject, lines),
    ...
    private void record(UUID tenant, ActorId actor, ChatModelResolver.Resolved selected, ChatModelExecutor.Accounting accounting) {
