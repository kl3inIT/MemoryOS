...
public final class WebTools {
    ...
    public WebTools(WebProviderClient client, WebConnectionService.Access access, ChatEvidence evidence,
                    Runnable active, SearchTasks.Scope work, Instant deadline, Consumer<ChatSearchEvent> events,
    ...
    private synchronized String run(String kind, List<String> inputs, Request call) {
        ...
        String id = "web-" + kind + "-" + UUID.randomUUID();
        events.accept(new ChatSearchEvent(id, ChatSearchEvent.Stage.STARTED, null));
        if (kind.equals("search")) events.accept(new ChatSearchEvent(id, ChatSearchEvent.Stage.SEARCHING, null,
                new ChatSearchEvent.QueryPlan(pending, new SearchFilters(Set.of(), null, null)), List.of()));
        try (var ignored = work.enter()) {
            ...
            for (var result : results) {
                ...
                var source = evidence.register("web:" + result.url(), number -> new ChatSource(number, null, null,
                        result.title(), 0, 0, List.of(), null, null, new ChatSource.WebLocation(result.url(), excerpt, Instant.now())), id);
                if (source == null) break;
                output.append(entry(result, text, source.citationId())).append('\n');
            }
            active.run();
            events.accept(new ChatSearchEvent(id, failures == items.size() ? ChatSearchEvent.Stage.FAILED : ChatSearchEvent.Stage.COMPLETED, null));
            return output.append(footer).toString();
        } catch (Exception failure) {
            active.run(); // Cancellation must propagate, not become an ordinary tool error.
            events.accept(new ChatSearchEvent(id, ChatSearchEvent.Stage.FAILED, null));
