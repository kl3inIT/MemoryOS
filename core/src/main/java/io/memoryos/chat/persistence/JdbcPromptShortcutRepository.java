...
public class JdbcPromptShortcutRepository {
    public record Shortcut(UUID id, String name, String content, boolean active, boolean isPublic, boolean hidden, long revision) {}
    ...
    public List<Shortcut> visible(UUID tenant, UUID actor, boolean includeHidden) {
        return jdbc.sql("""
                ...
                .query((row, ignored) -> new Shortcut(row.getObject("id", UUID.class), row.getString("name"), row.getString("content"),
    ...
    public List<Shortcut> publicShortcuts(UUID tenant) {
        return jdbc.sql("""
    ...
    public Optional<Shortcut> locked(UUID tenant, @Nullable UUID owner, UUID id) {
        return jdbc.sql("""
