...
public class VoiceSynthesisService {
    /** Longest text one request reads aloud; Onyx has no limit. */
    ...
    SpeechStream stream(VoiceConnectionService.Connection connection, String key, List<String> segments, double speed,
            Runnable release) {
        var stream = new SpeechStream(connection, key, segments, speed, release);
        try {
            stream.start();
            return stream;
    ...
    public final class SpeechStream implements AutoCloseable {
        ...
        private SpeechStream(VoiceConnectionService.Connection connection, String key, List<String> segments, double speed,
                Runnable release) {
            ...
            var model = OpenAiAudioSpeechModel.builder().openAiClient(client).options(options).build();
            chunks = Flux.fromIterable(segments)
        ...
        public void close() {
            ...
            try {
                ...
                try {
                    ...
                    release.run();
                    meters.timer("memoryos.chat.voice.request", "provider", connection.provider().name(),
                            "operation", "synthesize", "outcome", outcome).record(System.nanoTime() - started, TimeUnit.NANOSECONDS);
                }
            }
        }
    }
