...
public final class ChatPrompts {
    ...
    private static final String RUN_PYTHON_GUIDANCE = """
            ...
            Build a .docx with python-docx structure, not plain paragraphs: `add_heading(text, level=0)` for the title and level 1-3 for sections, `add_paragraph(text, style='List Bullet')` or `'List Number'` for lists instead of typing a bullet or a number, `table.style = 'Table Grid'` so a table has visible borders, one `add_paragraph` per paragraph instead of a newline inside one, and column names in the first row.
            ...
            Build a .pptx by writing a deck plan and rendering it, never by placing shapes yourself: the plan is JSON `{title, subtitle, author, date, confidentiality, theme (corporate|slate|emerald), language (vi|en), sources: [{label, detail}], slides}`, and a slide is one of `title`, `agenda` (`items`), `section` (`title`), `bullets` (`title`, at most 7 `bullets`, optional `lead`), `columns` (exactly 2 `{heading, bullets}`), `metrics` (2-4 `{value, label, delta}`), `table` (`columns` and `rows`), `image` (`path` to an image you saved first, optional `caption`), `quote` (`text`, `attribution`) and `closing`; every slide also takes `note` for speaker notes and `cites` with 1-based indexes into `sources`. Then run `render-deck plan.json deck.pptx` via subprocess: it applies the corporate template, chooses every type size so nothing overflows, adds footers, slide numbers and the sources slide, and prints one JSON line. An unknown field, an eighth bullet or text that cannot fit is refused by naming the slide and what to shorten — fix the plan and render again; do not fall back to building the deck shape by shape.
            The output suffix picks the format: `render-deck plan.json deck.pptx deck.html` writes both from one plan. Add the `.html` output when the user wants to read the deck in a browser or asks for something quick — it is a single self-contained file, needs no PowerPoint, and prints to the same pages — and keep the `.pptx` whenever they may edit or present the deck.
            After saving any .pptx, run `check-pptx` via subprocess with all such files in one call; it prints one JSON line per file whose `issues` name what a reader would see, such as text overflowing its box, an empty slide, type too small to read or leftover placeholder text. Fix what it reports and render again before answering.
