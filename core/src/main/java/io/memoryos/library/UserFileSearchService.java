...
public class UserFileSearchService {
    ...
    public List<FileHit> search(ActorId actor, TenantId tenant, Set<UUID> allowed, String query) {
        authorize(actor, tenant);
        if (allowed.size() > 4020) throw LibraryException.invalid("Too many files in search scope.");
        var scope = documents(tenant, actor, allowed);
    ...
    public io.memoryos.retrieval.SearchDocument read(ActorId actor, UUID file, UUID generation, int from) {
        ...
        if (document == null) throw LibraryException.unavailable();
        var result = search.fileDocument(actor, tenant, scope, document, generation, from);
        authorize(actor, tenant);
        if (!document.equals(documents(tenant, actor, Set.of(file)).get(file))) throw LibraryException.unavailable();
        return result;
    }
}
