...
public interface LibraryArtifacts {
    ...
    boolean update(TenantId tenant, ActorId owner, ChatLibraryFile.Source source, UUID id,
    ...
    boolean restore(TenantId tenant, ActorId owner, ChatLibraryFile.Source source, UUID id);

    /** Ends an artifact's trash window now; false when it is not in the trash. */
    boolean purgeNow(TenantId tenant, ActorId owner, ChatLibraryFile.Source source, UUID id);
