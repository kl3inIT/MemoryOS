 ...
 *
 * <p>The generated files and images are Chat's rows ({@code chat_file_artifact}, {@code chat_image_artifact}, and
 * {@code chat_session}/{@code chat_message} for a conversation's own files). This read model reads them by SQL, the
 * one place the library reads Chat's tables: a port could not page and sort across both without loading every row.
 * Every change to an artifact goes through {@link io.memoryos.library.LibraryArtifacts}, which Chat implements.
 */
@Repository
...
public class JdbcLibraryRepository {
    ...
    /**
     * Renames or stars an upload the owner's library shows. A rename rewrites the file's own name, so the listing,
     * name search, conversation cards and the download all use it at once; the bytes and object key never change.
     * Answers false when no such upload is listed. Generated files and images are Chat's rows and change through
     * {@link io.memoryos.library.LibraryArtifacts}.
     */
    public boolean update(TenantId tenant, ActorId actor, UUID id, @Nullable String filename, @Nullable Boolean favorite) {
        return jdbc.sql("""
                UPDATE chat_user_file f SET filename = COALESCE(CAST(:filename AS varchar), f.filename),
                    favorite_at = CASE WHEN CAST(:favorite AS boolean) IS NULL THEN f.favorite_at
                                       ...
                                       ELSE NULL END
                WHERE f.tenant_id = :tenant AND f.id = :id AND f.owner_actor_id = :actor
                  AND f.status NOT IN ('DELETING','DELETED')
