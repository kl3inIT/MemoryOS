...
CREATE OR REPLACE FUNCTION fence_google_selection_membership() RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
    IF TG_OP='DELETE' OR NEW.status<>'ACTIVE' THEN
        UPDATE google_drive_selection_operations SET status='CANCELLED',error_code='IAM_ACCESS_DENIED',
            completed_at=CURRENT_TIMESTAMP,claim_token=NULL,lease_expires_at=NULL
    ...
    RETURN NEW;
END $$;
CREATE TRIGGER fence_google_selection_membership BEFORE DELETE OR UPDATE OF status ON tenant_memberships
    FOR EACH ROW EXECUTE FUNCTION fence_google_selection_membership();
