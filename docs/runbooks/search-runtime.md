...
1. As deployment UID 1000, set the Search values from the environment's `.env.example`, then run `python3 infrastructure/opensearch/provision-search.py`. The script provisions Dashboards only when `MEMORYOS_OPENSEARCH_DASHBOARDS_PUBLIC_URL` is set, as staging does; production publishes no Dashboards, so it gets the node and admin certificates, the `memoryos-service` account and `health.curl`, and neither the Dashboards certificate nor an OIDC client secret. An issuer set where no Dashboards is published is refused rather than ignored. This atomically creates an outside-Git secret directory on first use; later runs preserve passwords/certificates and reconcile managed Security YAML. It refuses a partial secret set. Back up this directory securely. Leaf certificates are valid for five years and the authority for ten (`LEAF_VALIDITY_DAYS`, `AUTHORITY_VALIDITY_DAYS`). Renewal still replaces a leaf inside its last thirty days; the long life is so that a deployment whose renewal cron was never installed does not stop answering on an anniversary nobody wrote down. Production is such a deployment today: the daily renewal cron is installed by the Dashboards proxy installer, which production does not run.
...
6. As the existing root Docker operator, run `python3 infrastructure/opensearch/provision-dashboards.py`. It calls the supported Dashboards Saved Objects API over verified container-local HTTPS, selects `global_tenant`, and reconciles stable `memoryos-chunks` index-pattern and `memoryos-chunks-inspection` saved-search IDs. The index pattern carries the field list read from `_field_caps` with the admin certificate (piped into a temporary file inside the OpenSearch container), because the read-only inspector cannot save a refreshed list and Discover otherwise cannot resolve `updated_at`. Replays make no write when the controlled fields already match. The curl credential file is generated from the existing server password by `provision-search.py`, mounted read-only and never printed or passed as a process argument. Basic authentication exists only for this operator API path; unauthenticated browser pages still redirect to OpenID Connect by default. Do not write `.kibana*` directly.
...
NPM connects to Dashboards with HTTPS, verifying the certificate against the public Search CA and the `memoryos-opensearch-dashboards` hostname. Dashboards has a separate server certificate/key; it does not receive the OpenSearch node/admin keys. `memoryos_search_inspector` gives the owner a full read-only view: `read`/`indices_monitor` and index metadata reads on all indices, cluster monitoring, the get/search/list/explain APIs of Dashboards plugins such as Index Management, and `kibana_all_read` for the global tenant. It grants no write, delete or security-configuration action, and the human inspector mapping never includes `kibana_user`. Dashboards read-only mode is disabled (`readonly_mode.roles: []`) and the inspector is not mapped to `kibana_read_only`: that mode hides every application except Home and Dashboards, including the Discover inspection view, and never enforced write denial. The backend inspector role is the permission boundary. OIDC remains the default browser authentication route. The colocated Basic route is required only for the controlled Saved Objects call using the existing internal Dashboards server identity; its credential stays inside the protected secret mount.

The cluster is named by the environment that starts it: each `compose.search.<environment>.yaml` sets
`cluster.name` on the node and the same value as `MEMORYOS_SEARCH_CLUSTER_NAME` on the security bootstrap.
`opensearch.yml` names neither, because both environments mount it. `securityadmin.sh` refuses a name the
running node does not carry, so the two values must agree;
`infrastructure/opensearch/test_environment_neutral_search.py` holds that and the rest of this rule.

The proxy installer installs a daily 03:41 operator cron for `/apps/memoryos/renew-internal-search-certificates.py --renew-certificates`. This is the checked-in `provision-search.py` copied to a stable path. Renewal checks node/admin/Dashboards leaf certificates, replaces those expiring within 30 days, retains the CA/DNs and service credentials, and backs up the old leaf pairs under the protected secret directory. It restarts the affected OpenSearch/Dashboards containers, waits for health and restores the old pairs on reload failure. A renewed admin certificate uses the unchanged trusted CA and admin DN; `search-security-bootstrap` reads the current leaf on its next invocation, so renewal does not rewrite Security configuration. Run the same command manually for overdue renewal; `--within-days 366` forces a controlled rotation drill. The CA has a separate 10-year validity and must be replaced through a coordinated trust-distribution change. Keep renewal logs under `/apps/memoryos` and verify cron health during operational checks.
