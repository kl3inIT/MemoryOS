...
Before the first staging start, run `infrastructure/inspection/provision-staging-secrets.sh`. Copy its generated Redis worker password to Infisical without printing it; load the pgweb/Redis Insight client secrets, Redis inspector password, and Redis Insight encryption key into the controlled reconciliation/deployment shell. The script preserves complete secret sets, refuses incomplete Redis TLS material, and prints no secret values.
