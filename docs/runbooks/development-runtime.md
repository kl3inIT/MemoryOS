...
1. Run or debug `MemoryOS API Dev`. It uses module `memoryos.api.main`, starts `io.memoryos.api.MemoryOsApiApplication` on `18080`, creates PostgreSQL on `55432`, and owns Flyway. Wait for `GET http://127.0.0.1:18080/actuator/health` to report `UP`.
2. Run or debug `MemoryOS Worker Dev`. It uses module `memoryos.worker.main`, starts `io.memoryos.worker.MemoryOsWorkerApplication`, connects to the API-owned database, creates Redis on `56379`, and keeps the db-scheduler tasks plus ingestion and cleanup consumer loops running. Its Java `@argFiles` launch preserves the concrete runtime classpath inherited by the bounded Tika child JVM; the worker module's runtime-scoped `:connector` dependency supplies the real extraction provider.
3. Stop `MemoryOS Worker Dev` first so consumers and db-scheduler shut down while PostgreSQL is available, then stop `MemoryOS API Dev` so it can remove its PostgreSQL Dev Service.
...
## Run the API with manually supplied development values

The managed Infisical path above is preferred. For an isolated environment whose launcher injects values through another approved secret mechanism, use the equivalent development configuration shape:

```powershell
$env:MEMORYOS_IDENTITY_ISSUER = "https://<development-keycloak>/realms/<development-realm>"
$env:MEMORYOS_IDENTITY_JWK_SET_URI = "https://<development-keycloak>/realms/<development-realm>/protocol/openid-connect/certs"
...
$env:MEMORYOS_KEYCLOAK_ADMIN_SERVER_URL = "https://<development-keycloak>"
...
$env:MEMORYOS_TENANT_ID = "<stable development Tenant UUID>"
$env:MEMORYOS_INITIAL_OWNER_SUBJECT = "<stable development Keycloak user ID>"
$env:MEMORYOS_TENANT_SLUG = "<development-tenant-slug>"
$env:MEMORYOS_TENANT_DISPLAY_NAME = "<development Tenant name>"
$env:MEMORYOS_INITIAL_TENANT_CHANGE_REFERENCE = "<approved development/change reference>"
...
Before the first staging start, run `infrastructure/inspection/provision-staging-secrets.sh`. Redis and the worker consume the same generated worker-password file, and Redis and the API the same `api-password.txt`; load only the pgweb/Redis Insight client secrets into the controlled Keycloak reconciliation shell. Redis passwords, Redis Insight encryption key, cookie secrets, and TLS material remain file-backed Compose secrets. The script preserves complete secret sets, warns 30 days before certificate expiry, supports explicit coordinated rotation through `MEMORYOS_REDIS_TLS_ROTATE=true`, refuses incomplete TLS material, and prints no secret values.

Redis starts with TLS, `default` disabled, namespace-bounded worker and API (`memoryos:chat:stream:*`) ACLs, and a separate all-key read-only `memoryos-inspector` ACL for staging administration. The inspector can browse all data types and inspect connection/server/memory diagnostics, CONFIG GET and SLOWLOG GET/LEN, but cannot write/delete keys or mutate server/ACL configuration. Redis Insight stores encrypted state, cannot add/edit/delete connections, and receives only the preconfigured TLS connection. Its raw port is private; only `memoryos-redisinsight-oauth2-proxy` reaches the proxy network and loopback port `18027`.

Both proxies require realm role `memoryos-inspector`. Verify realm-local `admin` can enter each tool and an ordinary MemoryOS user receives denial. Then prove pgweb rejects a write transaction and the Redis inspector rejects `XADD`, `FLUSHALL`, and `CONFIG SET` while permitting `EXISTS`, all-key reads and `CONFIG GET`. Run `infrastructure/redis/verify-staging-inspector.sh` inside the staging Redis container to verify the ACL DRYRUN matrix and real inspector reads. Keep `/apps/memoryos-pgweb` stopped but intact for rollback until this acceptance passes.
