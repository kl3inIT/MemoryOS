...
- **API key.** The interpreter requires `X-Api-Key` on every `/v1` route, and the api sends the same key. Both read one host file, `/apps/memoryos/secrets/interpreter/api-key.txt`, mounted as the Compose secret `interpreter_api_key`.
