...
| PaddleOCR-VL answers map onto v2 blocks: page furniture dropped, labels to kinds, pixel boxes scaled to BOTTOMLEFT user-space points on portrait and landscape pages, a CropBox at (36, 48) placed where the viewer's `pdfBoxRect` draws it, a `/Rotate 90` page measured as displayed with `page_no` and no box, table HTML laid out with `rowspan`/`colspan` occupancy and no header flags, the Docling text grid, image input with page 1 and no box, mismatched or malformed answers refused, cell and column bounds | `PaddleOcrVlDocumentTest` (twelve cases; real PaddleOCR-VL-1.6 answers from the spike on the public HUT report, synthetic PDFBox pages) |
| The PaddleOCR-VL request carries the fixed parameters and a streamed base64 body with a declared length that decodes to the original bytes; HTTP 413/408/504/5xx/4xx, unparseable or error envelopes, the response cap, a refused connection, a timeout and interruption map onto typed failures without retry; with one permit a second request waits for the first and still succeeds with a timeout shorter than both together (the deadline starts at the permit), and the service never has more requests open than the bound | `PaddleOcrVlClientTest` (loopback `HttpServer`; no live PaddleOCR-VL) |
