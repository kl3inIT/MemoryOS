...
| PDF/DOCX/PPTX use Docling JSON; semantic text/table ordering and provenance survive without binary image data in normalized text; image-only, empty-table, over-limit and partial output are rejected; text bypasses Docling | `DoclingSourceContentExtractorTest`, including `excludesEmbeddedImagesAndPreservesSemanticContentAndProvenance`, `rendersTableOnlyDocumentInRowAndColumnOrderWithoutTextExport`, `rejectsImageOnlyDocumentEvenWhenTextExportContainsImageMarkup`, `rejectsTableWithoutSemanticCellText` and `rejectsSemanticTextOverCharacterLimitWithoutTextExport`; `SourceContentExtractorRouter` routing contract |
| When Docling fails for a reason of its own (unreachable, timeout, unusable response), a text PDF, DOCX, PPTX and a Chat attachment through the bounded spool are read natively and record `parser=tika`, `fallback_from` and `fallback_reason`; a scan-thin PDF keeps Docling's original failure; a failure of the document itself is not retried elsewhere; a Docling success carries no fallback fields | `DoclingFallbackTest` (seven cases, real Tika child process, PDFBox and POI fixtures; no live Docling) |
| PaddleOCR-VL answers map onto v2 blocks: page furniture dropped, labels to kinds, pixel boxes scaled to BOTTOMLEFT user-space points on portrait and landscape pages, a CropBox at (36, 48) placed where the viewer's `pdfBoxRect` draws it, a `/Rotate 90` page measured as displayed with `page_no` and no box, table HTML laid out with `rowspan`/`colspan` occupancy and the income statement's two header rows marked, the Docling text grid, image input with page 1 and no box, mismatched or malformed answers refused, cell and column bounds; a `figure_title` right above a table (past the unit note) is a level-2 heading while a running company-and-address title, a "(tiếp theo)" repeat, a signature caption and a title over text stay paragraphs; a chunk of a Paddle balance-sheet row reads `Section: … > BÁO CÁO TÌNH HÌNH TÀI CHÍNH` and `Số cuối kỳ: 2.470.482.178.999` without cell addresses | `PaddleOcrVlDocumentTest` (fourteen cases; real PaddleOCR-VL-1.6 answers from the spike on the public HUT report, synthetic PDFBox pages, `StructuredDocumentChunker`) |
| PaddleOCR-VL column headers on the leading rows of real HUT tables: one header row, a two-row header with `rowspan`/`colspan` (QUÝ I over Năm nay / Năm trước), two rows of spans over a blank corner, a section label that ends the header (also with "Kỳ trước" misplaced into it), a caption over the whole table, a group name in the item column left unmarked, header text with dates; the rule abstains for a caption over a mixed row, a header cell reaching into the data and a continuation table without header | `PaddleOcrVlTableHeaderTest` (ten cases; `hut-tables.json` holds the first rows of eleven public HUT tables) |
| The PaddleOCR-VL request carries the fixed parameters, chart recognition explicitly off for PDFs and images, and a streamed base64 body with a declared length that decodes to the original bytes; HTTP 413/408/504/5xx/4xx, unparseable or error envelopes, the response cap, a refused connection, a timeout and interruption map onto typed failures without retry; with one permit a second request waits for the first and still succeeds with a timeout shorter than both together (the deadline starts at the permit), and the service never has more requests open than the bound | `PaddleOcrVlClientTest` (loopback `HttpServer`; no live PaddleOCR-VL) |
| Scanned PDF and images go to PaddleOCR-VL; a text layer goes to Docling with `do_ocr=false` and `ocr=none`; without PaddleOCR-VL Docling keeps its OCR options and fingerprint; a PaddleOCR-VL failure or blank result is not replaced by Docling or Tika; Chat spool routing and the photograph-without-text case; image admission; an empty endpoint binds to an absent provider with a 30-minute timeout and two concurrent requests by default, and a concurrency bound of 0 or 17 is rejected | `PaddleOcrVlRoutingTest` (loopback `HttpServer`, mocked Docling transport) |
| Owned standalone Docling composition corrects orientation before layout only after independent-region consensus, preserves source bytes and abstains on disagreement/budget expiry; native-text pages and other pipelines are not rewritten | `infrastructure/deployment/ocr/tests/test_orientation.py` and `test_backend.py`; real API, sealed controls, canonical replay and resource/financial boundaries are recorded in the [service verification ledger](../increments/active/tasco-scanned-pdf-ocr/plan.md#approved-docling-service-implementation). Deployment and retained Source publication are not part of this verification |
...
| XLSX retains sparse coordinates, zero/false and cached formula values without evaluating or storing the formula; ZIP bombs and excessive sparse dimensions fail | `SpreadsheetSourceContentExtractorTest.xlsxRetainsZeroFalseSparseCoordinatesAndCachedFormulaValueWithoutTheFormula` and `xlsxRejectsDecompressionBombAndHugeSparseDimensions` |
...
| Sheets retain all accepted tabs, sparse/default/zero/false values and coordinates without storing formulas; incomplete windows, excessive represented cells and wrong snapshot versions fail | `GoogleNativeExtractionTest.sheetsPreserveSparseDefaultsZeroFalseAndAllTabsWithoutFormulas`, `rejectsIncompleteSheetWindowsAndOversizedRepresentedGrid`, and `rejectsWrongSnapshotVersionAndNormalizedOutputOverflow` |
...
| API/worker Spring task executors and real db-scheduler execution use virtual threads while configured workload, scheduler, datasource, and Redis bounds remain effective | `BearerAuthenticationIntegrationTest` and `ControlPlaneIntegrationTest.registersExecutesAndRecoversTheTopologyControlTask` |
| Flyway V7 creates the db-scheduler control plane, V8 creates Redis dispatch/processing evidence, and V9 cuts FILE binaries over to restrictive object-storage ownership against real PostgreSQL | Every PostgreSQL-backed test applies the full Flyway chain; `ControlPlaneIntegrationTest` and `WorkerFileProcessingIntegrationTest` exercise these tables at runtime |
...
| The worker composition contains the Redis consumer and no direct PostgreSQL polling executor or poll-delay configuration | Full source/configuration review and worker runtime integration (`RedisExecutionTopologyIntegrationTest`, `WorkerFileProcessingIntegrationTest`) |
...
| Provider adapter imports only public capability APIs | `SourcesDependencyRulesTest` |
...
## Connection failure classification and real Orca recovery — 2026-09-13
