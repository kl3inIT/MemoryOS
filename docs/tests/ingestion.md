...
| PaddleOCR-VL answers map onto v2 blocks: page furniture dropped, labels to kinds, pixel boxes scaled to points from the top left on portrait, landscape and `/Rotate 90` pages, table HTML laid out with `rowspan`/`colspan` occupancy and no header flags, the Docling text grid, image input with page 1 and no box, mismatched or malformed answers refused, cell and column bounds | `PaddleOcrVlDocumentTest` (ten cases; real PaddleOCR-VL-1.6 answers from the spike on the public HUT report, synthetic PDFBox pages) |
| The PaddleOCR-VL request carries the fixed parameters and a streamed base64 body with a declared length that decodes to the original bytes; HTTP 413/408/504/5xx/4xx, unparseable or error envelopes, the response cap, a refused connection, a timeout and interruption map onto typed failures without retry | `PaddleOcrVlClientTest` (loopback `HttpServer`; no live PaddleOCR-VL) |
| Scanned PDF and images go to PaddleOCR-VL; a text layer goes to Docling with `do_ocr=false` and `ocr=none`; without PaddleOCR-VL Docling keeps its OCR options and fingerprint; a PaddleOCR-VL failure or blank result is not replaced by Docling or Tika; Chat spool routing and the photograph-without-text case; image admission; an empty endpoint binds to an absent provider | `PaddleOcrVlRoutingTest` (loopback `HttpServer`, mocked Docling transport) |
