...
| Bound bearer actor without membership receives stable `ActorId`, null Tenant, and no capabilities | `BearerAuthenticationIntegrationTest.returnsEmptyTenantAuthorityForBoundActorWithoutMembership` |
| Bound bearer owner receives Tenant presentation, `OWNER`, and `INVITATIONS_MANAGE` | `BearerAuthenticationIntegrationTest.returnsDurableTenantAuthorityForBoundOwner` |
| Active owner/member projection, inactive rows, and active-Tenant ambiguity use durable Tenant data | `JdbcTenantAccessResolverTest` |
...
| Initial Tenant transaction creates or reuses one stable actor binding | `DefaultInitialTenantBootstrapperTest.createsTheExactInitialAggregateAndReplaysTheSameConfiguration` |
...
| The committed browser API snapshot describes nullable Tenant context and excludes non-API routes | `OpenApiContractTest.committedContractDescribesOnlyTheLiveBrowserApi` |
| Bound identity without active Tenant authority gains no application session | `SessionSecurityIntegrationTest.rejectsABoundIdentityWithoutTenantMembershipAndInvalidatesItsSession` |
...
| Authenticated shell renders server-derived Tenant role/initials and hides owner administration UI for members | `NewSessionPage` unit tests and `identity-shell.spec.ts` — `renders the authenticated application shell` and `hides owner UI and blocks member administration deep links without requests` |
...
| Internal application navigation preserves one browser document and authenticated/admin layout state without an identity refetch caused by route replacement | `identity-shell.spec.ts` — `keeps one document, identity session, and admin shell across internal routes` and `closes mobile administration navigation after a client route change` |
| Foreground identity synchronization and actor/authority changes purge private query/mutation state across boundary remounts | `ApplicationSessionBoundary` unit tests — `keeps the accepted authority fingerprint across boundary remounts` and `refetches identity on browser focus even while the query is fresh` |
| Identity, private-query, and private-mutation `401` responses purge private client state and converge to signed out | `ApplicationSessionBoundary` unit tests — unauthenticated identity/query/mutation cases |
