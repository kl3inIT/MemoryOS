...
| Bounded result snippets/literal highlights/friendly metadata, filters/paging/cancel/retry/empty/out-of-order requests, responsive dialog, section switching, Escape close and focus return, with extracted markup retained as text | `web/src/features/search/search-presentation.test.ts`, `web/tests/e2e/search.spec.ts`; browser with HTTP fixtures |
