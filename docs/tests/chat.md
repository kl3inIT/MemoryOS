...
| Usage answers no limit until one is set, only a model manager reads or sets limits, a limit of zero is refused, an over-limit upload answers `409 CHAT_STORAGE_FULL` before any URL is signed, and a per-person limit overrides the Tenant's and can be removed | `ChatSessionApiIntegrationTest.aStorageLimitIsAdministeredByModelManagersAndRefusesAnUploadBeforeItIsAuthorized` |
...
| The administration page saves the Tenant limit in whole MiB, sets and removes a person's own limit, and refuses anyone without model management | `web/src/features/chat/chat-storage-quota-page.test.tsx` |
...
| Without a recorded policy nothing is deleted; with one, conversations older than it are deleted and recent ones kept, and the preview counts them first | `ChatSessionPurgeIntegrationTest.theTenantRetentionPolicyDeletesWhatNobodyHasTouchedAndCountsItFirst` |
