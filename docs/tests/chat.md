...
| Archive is idempotent, keeps activity time, filters regular/archived/all lists and history search, stays readable and renamable; owner-only with CSRF, invalid status rejected | `ChatPersistenceIntegrationTest.createsPrivateSessionWithOneRootAndSharedDefaultPersona` (real PostgreSQL service) and `ChatSessionApiIntegrationTest.createReloadListAndHistoryUseRealPersistenceAndOwnerAuthorization` (Spring API, real PostgreSQL) |
| Thread list adapter: offset list of all statuses, initialization from the created session with retry, naming after the first answer, deferred RUNNING history; sidebar archive/unarchive-on-open | `chat-thread-list-adapter.test.ts` (unit, fake HTTP) and `chat-archive.spec.ts` (Playwright fixture server) |
