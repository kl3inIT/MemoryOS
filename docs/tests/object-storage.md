...
| Contract | Evidence |
| --- | --- |
| S3 authorization signs method, key, media type, checksum, and size; MinIO accepts the exact request and rejects altered bytes or a different `Content-Length` | `S3ObjectStorageIntegrationTest.presignsVerifiesStreamsAndIdempotentlyDeletesObjects`, `rejectsContentThatDoesNotMatchTheSignedChecksum`, `rejectsContentLargerThanTheSignedSize` |
...
| Cleanup/verification races are fenced; a failed deletion does not abort later claimed rows and is retried only after lease expiry | `ObjectUploadLifecycleIntegrationTest.expiredVerificationClaimIsFencedFromCleanupCompletion`, `oneDeleteFailureDoesNotAbortLaterRowsAndIsRetriedAfterCleanupLeaseExpiry` |
...
| Staging MinIO Console uses exact Keycloak OIDC callback and owner-only claim mapping; owner access is bucket-read-only, ordinary users receive no storage policy, port `9001` is not host-published, and production renders no Console/OIDC configuration | MEM-56 Compose, Keycloak token, MinIO STS/S3, and browser acceptance evidence |
