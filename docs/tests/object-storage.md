...
| Inspection/streaming return SHA-256 metadata; a byte range past the end stops at the end and reports the whole size; deletion is idempotent and missing objects are typed | `S3ObjectStorageIntegrationTest.presignsVerifiesStreamsAndIdempotentlyDeletesObjects` |
