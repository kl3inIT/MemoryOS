...
| Browser groups configured Sources by provider in a semantic expandable table, navigates rows to dedicated detail, renders the compact catalog and vertical provider-owned FILE wizard, and preserves failed-finalization recovery across Source routes without another PUT | `identity-shell.spec.ts` FILE source scenario |
