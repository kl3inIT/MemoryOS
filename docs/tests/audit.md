...
| --- | --- |
| An event belongs to its change's transaction; a failed write leaves the change committed and is counted; details are limited to the declared fields; nothing rewrites or removes an event outside the retention sweep; a refusal survives its rollback; every action reads back from its value | `core/src/test/java/io/memoryos/iam/audit/AuditTrailTest.java` |
| Pages are stable while events arrive; filters narrow the stream and stay inside the Tenant; `%` is searched literally; reading and exporting need `AUDIT_READ` and exporting is recorded; retention deletes only expired events | `core/src/test/java/io/memoryos/iam/audit/AuditLogTest.java` |
