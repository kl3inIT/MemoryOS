...
- Capability persistence lives under the owning capability's `persistence` package. Use Spring Data JPA repositories for entity lifecycle CRUD, declarative queries and ORM locks; use concrete `@Repository` classes for SQL projections, claims and other custom mechanics. Group repositories by aggregate, use case, projection, or consistency boundary; do not create one repository per table mechanically.
