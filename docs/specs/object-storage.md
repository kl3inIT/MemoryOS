...
Deployment bootstrap owns bucket creation, private access, the readiness sentinel, CORS, service users, and least-privilege policies. API credentials may sign PUT and inspect `raw/*`; worker credentials may read, inspect and delete `raw/*`, and write, read and delete `extracted/*`. Both may read the configured readiness sentinel. Neither identity can list the bucket. API and worker readiness open a one-byte range from the sentinel and never mutate or list at startup.
