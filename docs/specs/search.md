...
MEM-46 implements direct document Search; verification and acceptance boundaries are recorded in the [completed increment](../increments/completed/mem-46-search/verification.md). Chat is documented separately under MEM-11.
...
For example, ranked hits `11, 40, 10` become section `10–11` anchored at match `11`, then section `40`. Adjacent chunks are concatenated with newlines, presented inside one document card and paged by document. Existing public-source eligibility is preserved; no additional permission model is introduced.
