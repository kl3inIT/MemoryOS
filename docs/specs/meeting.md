...
The job runs in the API beside the minutes, on the same scheduler, with a one-hour lease and three attempts; both jobs, like the Worker's export, archive and usage-report jobs, take the same leased-job pass (`LeasedJob`: fail what was abandoned, claim, run, record the failure against its attempt). The file is handed to the provider in the container it arrived in: MemoryOS decodes no audio. It is streamed from object storage into the provider's multipart upload in small reads with an exact `Content-Length`, so a 500 MB recording is never held in memory. **Soniox** transcribes up to five hours with `enable_speaker_diarization`, and its tokens are grouped into utterances at a speaker change, an 800 ms pause or 400 characters, as the live adapter groups them. An **OpenAI-compatible** connection answers `verbose_json` segments for a single speaker and caps the file at 25 MB. ElevenLabs and Azure transcribe dictation only and are not listed for a recording. A replica that dies during the last attempt does not leave the meeting `TRANSCRIBING`: before claiming, the job fails any recording whose last attempt's lease lapsed with `MEETING_RECORDING_FAILED` and ends its meeting, as a failed last attempt does.

The segments become `MIC` utterances, written with their speakers in two statements however many there are, the meeting ends, and its minutes are queued as for a live meeting. The recorded length the provider reported is added as `SPEECH_TO_TEXT` usage — not the file's size, because compressed bytes say nothing about seconds.

**The audio is deleted** once the recording is done with — its transcript stored, its attempts run out, or its last lease lapsed — and when the meeting is deleted. Those outcomes commit first and leave the upload on the meeting row; every pass of the recording job then sweeps the meetings whose `audio.status` is `DONE` or `FAILED` and still hold an upload (Flyway V129 partial index), retires the upload and only then forgets it on the row, conditionally on that same upload. The meeting row is the durable marker, so a process that stops between the two steps leaves the row for the next pass instead of an adopted upload nothing retires; an upload an earlier pass already retired is only forgotten, and any other failure is retried on the next pass. Deleting a meeting retires its adopted upload before the row goes, and refuses the delete if that fails, because afterwards nothing would point at the bytes.
...
One call to the Tenant's model for `MEETING_MINUTES`, no tools and no conversation, produces a structured `summary`, a `kind`, `decisions` and `actions`. The transcript reaches the model as numbered lines (`[n] time Speaker: text`) under the title, the participants, the meeting time and the owner's notes; speakers and times read exactly as in the transcript export (below), which the correction prompt also uses. Past 120,000 characters both ends are kept and the model is told part of the meeting is missing. The prompt forbids inventing a decision, an action, an owner or a date, requires an explicit assignment before work becomes an action, refuses to assign work to a group, and treats the transcript as untrusted data.
