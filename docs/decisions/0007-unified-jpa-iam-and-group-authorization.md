...
The user approved a cohesive IAM model, JPA lifecycle persistence, Account Type classification and reference implementation-aligned Users/Groups interactions. Existing MEM-55 work must be retained, and both issues are delivered on one branch. Sources, documents, object storage and worker ingestion remain separate capabilities.
...
- reference implementation informs the interaction model and permission semantics, not speculative product controls, legacy roles, upstream authentication storage or unmeasured caching.
