...
- Inside the jar the adapters use `ingestion.extraction.StructuredContent` and, as before, `ingestion`'s extraction exceptions, while the extractor router constructs the Google Docs, Google Sheets and SharePoint page extractors. `core`'s Modulith verification does not see the bundle, so this `connector.adapter` → `ingestion` use is recorded here, not enforced; `SourcesDependencyRulesTest` still forbids the bundle from any `application` or `persistence` package.
