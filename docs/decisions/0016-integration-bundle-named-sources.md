...
- 2026-09-25, phase 3: [ADR 0015](0015-capability-module-map.md) placed the two sync engines in `connector.sync` without merging them. Phase 3 merged them: `DefaultConnectorSyncService` and `DefaultSharePointSyncService` are replaced by `connector.sync.SourceSyncEngine` (a `SyncRun` per execution slice of an attempt), and each provider supplies only its `SyncTraversal` (`GoogleDriveSyncTraversal`, `SharePointSyncTraversal`). The provider adapters in `connector.adapter` are unchanged by this; the [connector spec](../specs/connector.md#synchronization-engine-and-failure-semantics) owns the engine's failure semantics.
