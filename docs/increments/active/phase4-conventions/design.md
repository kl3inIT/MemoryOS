...
Owner decision 2026-09-25 (group 4 of [owner-decisions.md](../audit-quality-fixes/owner-decisions.md) plus the listed follow-ups); best practice even where contracts change; one pull request.
...
- **Ownership follow-ups.** An identity-owned people-and-Group search replaces `/api/chat/persona-share-options` for the web principal picker; the Google Drive account OAuth client and sign-in admission decisions move from `api` into their capabilities; branch and meeting-publish endpoints get API tests.
