...
2. **Crop helpers** — `chat-image-crop.ts`: `rectBetween` (a drag in either direction, clamped to the
   image), `cropPixels`, `cropType`, `croppedFileName`, `renderCrop` (canvas). Done.
3. **Cropper** — `chat-image-cropper.tsx`: drag over the image, everything outside the selection dimmed,
...
| What | Where |
| --- | --- |
| A drag in either direction, clamped to the image; the crop in the image's pixels; the saved type and name | `chat-image-crop.test.ts` |
| Crop mode selects a region, reports `640 × 360 px`, saves a `… (đã cắt).png` file, and reports a failed encode; only a download where nothing can be saved | `chat-file-preview-crop.test.tsx` |
| The page size re-cuts the list from its first page, with the summary | `chat-library-page.test.tsx` — "cuts the list into pages of the chosen size" |
| A question opens a conversation with the file attached, copying nothing for an upload | `chat-library-page.test.tsx` — "asks Chat about the file being previewed" |
| The file is attached before the question is sent; nothing is sent when the file never arrives; a file with no question | `chat-conversation-seed.test.tsx` |
