...
- Layout: at the large breakpoint the rail sits left of the results and the results column keeps its readable width; below it the rail becomes a Sources menu in the filter toolbar. The page never shows both. The rail is hidden when fewer than two connectors have results and none is selected.
