...
| FeaturesGrid (bento) of icon cards | Capability explorer: one markup mock of the product per capability, pinned and switched by scroll on roomy desktops | Landing page 6.0: fewer words, and each capability shown working instead of named beside an icon |
...
- **How it works** lights the beam station by station with the scroll; on roomy desktops the list is pinned and snaps to the end of each station. Each station has one progress value, `--p`, and the `ramp` utilities in `src/styles/base.css` turn it into each part's fade, movement, growth or stroke drawing, so the whole story runs six tweens. A finished station keeps a quiet CSS loop while the list is on screen: light runs along the lit beam, packets reach the page, the scan sweeps again, the cuts flash, the vector bars shift, the index pulses, and the question keeps reaching the passages it cites. The access gate then assembles: its links grow, passages fly from the person to the check, and the blocked ones are struck through.
- **Capabilities** pins the explorer: its list follows the scroll, and the active panel crossfades in and plays its mock. Mocks declare their entrances as data attributes, and one interpreter builds a mock's timeline when it is first needed.
...
Pinning needs a viewport of at least 64 × 44 rem; smaller screens scrub or play the same scenes in place. The markup always holds the final state: GSAP sets start states only while `prefers-reduced-motion: no-preference` matches, so reduced motion, a failed script and the component tests all render the complete static page. Only transient marks — scan lines, flying copies, packets, the beam's light and the caret — are hidden in the markup.
