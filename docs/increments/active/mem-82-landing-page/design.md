...
- Organization **Vadan** (`vadan.app`, contact `aws@vadan.app`); product **MemoryOS** ("MemoryOS by Vadan"); **Backed by GenAI Fund**. English only.
...
- Tasco is named as Vadan's partner in the Production PoC (September–December 2026), never as a customer. No Tasco pricing, internal architecture, users or data appear.
- The site serves the apex `vadan.app` from the existing staging VPS, replacing the current Cloudflare redirect to `roll-bits.com`. `www.vadan.app` redirects to the apex. The Google Workspace MX records stay unchanged.
...
1. **Header** — MemoryOS mark, anchors (Product, How it works, Deployment, Roadmap, FAQ), light/dark theme toggle (inside the menu below `md`), Contact action (`mailto:aws@vadan.app`), skip link.
...
9. **FAQ** and **Footer** — the FAQ carries the technical detail the sections no longer spell out; the footer holds the contact call to action over a second particle field and © Vadan. It links neither the source repository nor the third-party notices; `/THIRD_PARTY_NOTICES.txt` stays served.
...
- Replacing the apex redirect changes current public behavior of `vadan.app` (owner-approved 2026-09-11).
