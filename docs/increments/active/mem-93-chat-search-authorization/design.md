...
- **Legacy chunks and backfill.** Chunks written before the access fields existed match the filter until V51's `ACCESS` backfill rewrites them; the post-query database recheck authorizes every hit meanwhile. An `ACCESS` operation rewrites `source_metadata`, `metadata_hash` and the access fields with `_update_by_query`; it never marks the document pending, never embeds, and retries briefly when the document is waiting for a content rewrite (which reads access itself).
