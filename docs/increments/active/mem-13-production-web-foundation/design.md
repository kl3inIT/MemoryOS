...
The existing external reverse proxy sends the complete MemoryOS HTTPS origin to `memoryos-web`. Its Nginx runtime serves the SPA and proxies backend-owned paths to `memoryos-api` on `shared-infra`; Docker DNS is resolved at request time with a bounded TTL so an API-only container replacement does not strand Nginx on the old address. Local Vite development proxies the same paths, preserves the browser host and forwarded scheme, and removes only the production `Secure` cookie attribute at its loopback-only HTTP boundary so the OAuth state session survives the callback. Production never performs this rewrite. CORS is not introduced.

Frontend quality and browser checks run inside the exact `mcr.microsoft.com/playwright:v1.62.1-noble` image matching the pinned test package. Browser binaries and Linux libraries therefore come from one immutable image instead of an `apt` transaction on every runner. A local gate fails when the package and image versions diverge, while Dependabot covers npm, Docker images, GitHub Actions, and Gradle. Browser tests own an isolated Vite server on port `4173` and a loopback backend on an ephemeral port, so the documented development services can remain running. The production web image builds in a separate parallel Ubuntu job so Docker validation remains independent of the browser container. Playwright HTML reports are required CI artifacts for both failures and successful runs.
