...
- [ ] 1. `V96__ai_usage_limit.sql`: the table, its checks (at least one budget, `group_id` only for `GROUP`,
...
- [ ] 2. `io.memoryos.usage`: `AiUsageLimit`, `AiUsageLimitScope`, `AiUsageLimitRepository` (CRUD, and the SUMs the
...
- [ ] 3. Refusal in `ChatTurnService.sendLocked` and in session naming (swallowed there), the new failure code in
      `FAILURE_CODES`, and `ChatException.limitExceeded` carrying the scope and reset instant.
- [ ] 4. HTTP: 429 with `Retry-After` and a problem body naming the scope and reset; admin CRUD under
      `/api/ai-costs/limits` on `MODELS_MANAGE`; the person's own binding limit on the existing `/api/ai-costs/mine`.
- [ ] 5. Audit: new `AuditAction` values for a limit created, changed and deleted, recorded in the change's
      transaction, with the before and after budgets as details.
- [ ] 6. Web: the Limits section on `/admin/ai-costs` with limit beside usage, the create and edit dialog, and the
      banner on Settings › Usage. Vietnamese copy keeps `token`, `model` and `provider` in English.
- [ ] 7. Tests: the repository's window arithmetic, a turn refused at the Tenant, Group and person scope, a turn
...
- [ ] 8. Docs: extend [AI usage](../../../specs/ai-usage.md) and its
