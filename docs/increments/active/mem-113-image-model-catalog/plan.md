...
- [ ] Tests:
  - catalog invariants (done in `ImageProviderTest`);
  - authorization: 403 without `MODELS_MANAGE`;
  - required endpoint;
  - OpenAPI contract (path added to `BROWSER_API_PATHS`).
...
- [ ] Check the composer's image toggle when `available=false`.

## Step 3 — `shape` for `generate_image`

- [ ] Replace `size` with `shape`, mapped through the catalog `sizes`. Update the `ChatPrompts` guidance; add tool and adapter tests. `edit_image` is unaffected.
...
- [ ] Update `docs/specs/chat.md` (Image generation) and `docs/tests/chat.md`. `./gradlew clean check`, `pnpm check`, and CI green.
