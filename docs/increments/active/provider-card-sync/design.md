...
- Web search moves to the wide settings layout and the Models grid: hosted search APIs first, then "Gateways & self-hosted" (9Router, SearXNG). The crawler section and provider-hosted search use the same card and grid. Section headings use the `font-heading-h3` token.
