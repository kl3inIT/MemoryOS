...
- [x] Browser verification desktop + narrow widths
- [x] Polish pass: `StatusBadge size="sm"` for dense lists, single-line run rows, ACL detail header merged (one refresh button), carded links section, smaller tree expand button, bolder summary counts
- [x] Saved links auto-load (no button); read-only links hide redundant label/count; search placeholder/help clarify roots+linked scope
