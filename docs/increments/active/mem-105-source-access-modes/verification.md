...
- `SourceAccessModesMigrationTest`: V56 renames RESTRICTED to PRIVATE, accepts only PUBLIC/PRIVATE/SYNC, and makes `access_type` on Drive selection operations nullable.
...
A local API, worker and browser bundle ran against a fresh Arconia PostgreSQL migrated through V56. The owner signed in through the remote Keycloak and authorized Google Drive; no staging permission was changed and no email address is recorded here.
