...
- JetBrains MCP inspected all five changed TypeScript/TSX files with warnings enabled and reported no problems. LSP reported no diagnostics across the Source feature; its standalone E2E-file project reported the existing missing Node-type environment, while repository `tsc -b` and Playwright compiled that file successfully.
