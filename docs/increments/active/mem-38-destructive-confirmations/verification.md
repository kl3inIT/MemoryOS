...
| Starting a destructive mutation clears stale page-level feedback before dialog-local feedback is presented | Playwright first injects a reindex 409 and asserts the page alert, then confirms removal, injects a remove 409, and asserts exactly one remaining alert with the removal-specific dialog message. |
