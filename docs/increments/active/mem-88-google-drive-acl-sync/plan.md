...
- [x] Drop `google_drive_membership.content_provider_version` from V61 before merge, with the `unchanged()` fallback, the five-argument `observe` and the `releaseConfirmed` COALESCE.
