...
- `ChatSessionApiIntegrationTest.searchChatHistoryUsesIndexesAllVersionsAndOwnerFilteredPagination` passed against real PostgreSQL (Testcontainers) with Flyway V50 (authored as V44, renumbered after main added V43–V48) applied: case-insensitive `simple` token matching in titles and unselected historical ASSISTANT versions, `limit`/`offset`/`hasMore` pagination without duplicates, deleted session exclusion, another actor sees nothing, special characters return no error, >200-character query / `limit=51` / `offset=-1` rejected with 400, search does not select or mutate branches, a >64 KiB answer with 100,000 distinct tokens is stored and still matched at its tail, both GIN indexes exist, and inactive Tenant membership returns 403.
- `:core:compileTestJava :api:compileTestJava` passed. JetBrains `get_file_problems` (warnings included) reported no problems for `JdbcChatSearchRepository`, `DefaultChatSessionService`, `ChatSessionService`, `ChatSessionController` and `ChatSessionSearchResponse`. V50 (authored as V44, renumbered after main added V43–V48) reports only IDE-without-data-source findings: "no data sources configured" and unresolved `deleted_at`, which V36 adds to `chat_session`; the PostgreSQL test applied V50 (authored as V44, renumbered after main added V43–V48) and asserted both indexes. `openapi.yml` inspection timed out twice (5,600+ lines); `OpenApiContractTest` and `check:api` cover that contract.
