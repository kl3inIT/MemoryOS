...
**Nguồn dữ liệu:** chưa nhận dữ liệu Tasco thực tế. Các Google documents của những lần nghiệm thu trước đều tự tạo/mô phỏng; provider thật không đồng nghĩa dữ liệu Tasco. Trạng thái preservation V21 và các gate/live runs cũ dưới đây là lịch sử. MEM-76 không restore baseline ba roots: folder `1. VETC` và selection người dùng đã sửa phải được giữ nguyên. Không dùng history fixtures để tuyên bố Google capacity hoặc live discovery → approval → synchronization đã đạt.
...
- Giữ Source/Item/current Document, singleton Tenant, Owner authorization, PostgreSQL authority, Redis delivery, MinIO và provider bundle hiện có. Không dựng generic workflow/audit framework, queue platform hay pipeline Drive thứ hai.
...
- Chỉ activation transaction mới kiểm tra lại Tenant → credential → Source, owner hiện hành, revisions, mode và kết quả verification; atomically thay roots + approvals, tăng scope revision và fence/schedule theo contract hiện có. Invalid/inaccessible/overlapping/oversized/stale/cancelled proposal không thay active roots, lịch hoặc Document. Credential revoke/reconnect/deletion và source deletion phải xử lý cả pending intents.
...
- V22 lưu selection intents/receipts/checkpoints; V23 thêm nullable legacy run metadata, exact child attribution, counters, retained safe errors và indexed history/retention queries; V24 thêm Files keyset/latest-attempt indexes. Không sửa V1–V21 hoặc reset database người dùng. Legacy Unknown là bằng chứng thiếu thật, không phải execution shim. HTTP/OpenAPI/generated clients và worker dispatch cut over cùng đợt; final drift/backend/frontend gates đã thực thi và ghi trong plan.
...
- Persistence sở hữu SQL/cursor/index; application giữ Owner/Tenant authorization. Bound candidate rows trước version/latest-attempt projection để không join/render toàn corpus rồi mới limit. Migration additive V24 chỉ bổ sung index cần thiết.
...
- Create và replace-root contract bắt buộc có `scopeMode`; mode được chọn một lần khi tạo Source. `/google-drive/roots` chỉ chấp nhận mode đang lưu, không cho General ↔ Specific; từ chối trước khi gọi Google hoặc thay đổi state. Specific vẫn cho sửa links với scope revision/If-Match và tenant/owner/CSRF guards. Persistence không cập nhật mode khi replace roots; việc thay links vẫn cancel/fence generation và publication cũ trước reconciliation.
