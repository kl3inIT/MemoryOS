...
- [x] Integrate main `287ca9c` (Chat/JIT), then the user-requested fresh main `3f236d5` (Search PRs #83/#87), retaining HEAD Source behavior and regenerating the combined API/client.
- [x] Verify the combined tree with fresh build outputs and disposable test databases, without modifying the running review database or its Flyway history. Final uncached repository gate passed in 10m18s; exact module counts, conditional skips and frontend/browser proof are in the linked publication verification.
- Publication receipt: push this feature branch only after the final gate; record exact remote SHA and commit links in MEM-76 after remote verification, without opening a PR or changing issue status.
...
The follow-up fetch found main `3f236d5`: Search workspace/filter/preview changes, semantic-candidate threshold and test/cache corrections. The earlier `287ca9c` integration passed shifted migration fixtures and API generation (5m15s), `pnpm check` (83 tests in 17 files), and read-only Source/Chat smoke. Those results are intermediate evidence, not substitutes for the newest main gate. Final refreshed-tree browser/frontend results and boundaries are recorded in the [publication verification](../../../tests/connector.md#publication-main-refresh--2026-09-09).

The first full refreshed-tree backend attempt exposed verification-fixture issues: the isolated output root lacked ArchUnit's standard `/build/classes/.../test/` shape, so architecture checks imported test classes; preserve that suffix inside the isolated root rather than changing the architecture rules. The Search retry/expired-claim fixture also failed while requesting redispatch at database `CURRENT_TIMESTAMP`; set its two forced redispatch times one second in the past, following the existing PostgreSQL lifecycle-fixture convention. Keep the original retry/fencing assertions and production code unchanged. Rerun the complete uncached gate, without excluding tests or increasing their deadlines.

A subsequent attempt exposed `ConcurrentModificationException` in Spring's `MockHttpServletResponse` header map during committed Chat SSE replay. The replay and deadline-outcome cases now use the test's existing random-port HTTP server, signed-token helper and real `HttpClient` instead of concurrent MockMvc streaming; cursor/owner rejection assertions remain, as do all replay/outcome/header assertions. Production streaming and security code are unchanged. The final gate uses `--continue` to execute all modules while still failing if any task fails.

The final fetch before publication confirmed `origin/main` is still `3f236d55a99299df3f1741fe997dc71e1339f4dd`, identical to the verified merge input. All verification-owned browser tabs and fixture servers were closed; the throwaway build-isolation init script was removed after the successful gate. Sanitized ignored screenshots and test reports remain as local evidence. Existing review runtimes and databases were not cut over.
...
User-approved UI revision — 2026-09-09: remove the Source Run history component, its list/detail/error requests and dedicated browser scenario. Replace the verbose item-processing list with a compact Indexing attempts table and server-side page sizes 5 (default), 10, 25 and 50. Keep backend run contracts/data/retention, synchronization controls, selection and Files unchanged. Verify desktop/mobile rendering, page-size changes and cursor reset; the earlier 39-scenario evidence below describes the pre-removal UI.
...
- [x] Confirm the OAuth/service-account contract: OAuth JSON belongs to a credential; explicit URL selections disable account-wide retrieval.
...
- [x] Clarify scope: align existing MemoryOS screens without adding new Chat/Search/Agents capabilities.
...
- [x] Align shared tokens/primitives, responsive shell, session/access and invitation screens with the accepted MemoryOS interaction contract, retaining existing authorization and functionality.
...
- [x] Implement the approved setup sidebar, credential card/table, OAuth modal and compact upload/paste component, replacing the custom full-page form while retaining Source-owned OAuth semantics.
...
- [x] Restore the four approved metadata fields (ID, Name, Created, Last Updated), replacing the overly reduced picker; retain compact responsive rendering and disclosed lifecycle actions.
...
The user has approved reusable Google Drive credentials for multiple Sources. The prior Source-owned UI adaptation is superseded by the independent credential lifecycle in the updated design. MemoryOS keeps its existing tables while replacing the Google singleton constraints, atomic credential-plus-Source OAuth creation and orphan-credential cleanup.
...
- [x] Bind Select credential/Create New/Connector steps to real metadata and independent Source creation; migrate all callers.
...
Local runtime preservation: retain Source `45a91ebf-c73d-47ba-95f6-04e96f7cf808` and its three indexed Sheets. Do not edit the frozen `D:/Capstone Project/MemoryOS` donor. No PR, push, merge or deployment.
...
| `sources/.../connector/adapter/googledrive` | `RestGoogleDriveProvider`, properties/auto-configuration, `NativeSnapshot`, `GoogleSheetsSourceContentExtractor`, `GoogleDocsSourceContentExtractor` | Bounded native acquisition and offline structural extraction; replace donor aggregate Changes and manifest control flow |
| `sources/.../ingestion/extraction` | `SourceContentExtractorRouter`, `SourceContentExtractorAutoConfiguration`, `StructuredContent`, `SpreadsheetSourceContentExtractor` | Native descriptor first, detected binary format second; XLSX/CSV Java tables; retained Docling PDF/DOCX/PPTX and Tika UTF-8 text leaves |
...
The user-reported missing feedback and permanently displayed interval acknowledgement are the reproduction evidence. Source detail now uses the existing Radix dependency for page-scoped notifications and a shared abortable observer for the exact returned operation. The live Source announced Synchronization requested and Synchronization complete, explicitly allowing indexing to continue; a same-value interval save announced Automatic interval saved for one minute. Further setup changes remained paused while General/Specific scope was reviewed.
...
## Creation-only scope correction

Owner approved removing post-creation General/Specific switching. Scope controls appear during creation and connector configuration is read-only in detail. This supersedes the mode-switch acceptance above, not the recorded historical test evidence. Specific link editing remains supported; mode, credentials, existing content and schedules are not migrated.
...
- [x] Verify table/counter semantics against MemoryOS Source-run and per-file contracts.
