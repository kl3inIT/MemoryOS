...
Phase 3: [phase3-dedup-hot-paths](../../completed/phase3-dedup-hot-paths/design.md); phase 4: [phase4-conventions](../phase4-conventions/design.md).
...
- [x] **Cùng loại** (làm 2026-09-25, [phase 4](../phase4-conventions/design.md): Google account OAuth client về provider bundle, admission đăng nhập quyết định trong IAM; `MeetingController.minutes/publish` chỉ còn một lời gọi service): `GoogleDriveAccountClient` và `GoogleDriveOAuthProperties` (đọc tay `memoryos.google-drive.*` trùng `GoogleDriveProviderProperties`), logic admission trong `ActorSessionLoginSuccessHandler`, orchestration trong `MeetingController.minutes/publish`.
