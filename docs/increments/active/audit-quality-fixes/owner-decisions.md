...
Phase 3: [phase3-dedup-hot-paths](../../completed/phase3-dedup-hot-paths/design.md); phase 4: [phase4-conventions](../../completed/phase4-conventions/design.md).
...
- [x] **Provisioning catalog/persona của Chat.** Duyệt 2026-09-24 (best practice: provision khi tạo Tenant). Đã làm trên nhánh `dathip04/chat-provision-on-tenant`: IAM phát `TenantBootstrapped` trong transaction bootstrap, Chat provision catalog, flow và agent mặc định bằng `@EventListener` cùng transaction; lần khởi động lại provision Tenant cũ; các method đọc của catalog là `readOnly`. Xem [Chat defaults provisioned with the Tenant](../../completed/chat-provision-on-tenant/design.md).
- [x] **Provider OpenAI (~1.600 dòng) nằm trong `api`.** Duyệt 2026-09-24: chuyển về `core` tại `chat.catalog.openai`, không đổi hành vi, xem [OpenAI provider in core](../../completed/openai-provider-in-core/design.md). Quy ước: api/worker chỉ là composition root. Hệ quả: minutes và transcription của meeting buộc chạy trên API.
- [x] **Cùng loại** (làm 2026-09-25, [phase 4](../../completed/phase4-conventions/design.md): Google account OAuth client về provider bundle, admission đăng nhập quyết định trong IAM; `MeetingController.minutes/publish` chỉ còn một lời gọi service): `GoogleDriveAccountClient` và `GoogleDriveOAuthProperties` (đọc tay `memoryos.google-drive.*` trùng `GoogleDriveProviderProperties`), logic admission trong `ActorSessionLoginSuccessHandler`, orchestration trong `MeetingController.minutes/publish`.
...
- [x] (phase 3) **Sync engine Google/SharePoint** (fencing, settle, acquire/adopt/remove, attempt persistence, selection repo/processor, HTTP plumbing, audit helper) — hai bản đã lệch nhau về pause, retry, logging. Đụng MEM-126.
- [x] (phase 3) **`PdfText`** dùng chung cho `MeetingTranscriptPdf`, `MeetingMinutesPdf`, `UsageReportPdf` (font, `safe()`, wrap). Đụng MEM-92.
- [x] (phase 3) **Ba admin service image/web/voice connection** gần như giống hệt.
- [x] (phase 3) **Hot path OpenSearch:** `ensureIndex()` 5–8 HTTP/document và `synchronized`; HEAD alias trên mọi search. Cache "đã đảm bảo" theo process sẽ không nhận ra index bị xoá từ ngoài — chấp nhận không?
- [x] (phase 3) **Send path của Chat:** gộp `persona()` (3 lần × 5 query), `effectiveCapabilities` (≥4 lần), đọc file từng cái trong lock.
- [x] (phase 3) **SharePoint liệt kê lại toàn bộ site mỗi slice 45 s**; ~25 lock query/file khi sync; Google walk cha cho từng file.
- [x] (phase 3) **Cursor `scan` dạng chuỗi nối** trong `JdbcDocumentChunkRepository` → row-value cursor (đổi format cursor đang lưu).
...
- [x] (phase 4) FQN inline → import (~700 dòng: core ~420, api ~90, worker ~30…).
- [x] (phase 4) Log positional → fluent `event`/`error_code` (~100 chỗ).
- [x] (phase 4) Bỏ ~71 header `Cache-Control: no-store` và ~15 `nosniff` thừa (Spring Security đã gửi).
- [x] (phase 4) Chuyển record lồng trong controller (`MeetingController` 34, `ChatLibraryController` 13, `ChatEventStream` 13…) sang `contract/` — cần kiểm tên schema OpenAPI không đổi.
- [x] (phase 4) `ApiProblem` khai báo `additionalProperties: false` nhưng handler thêm `scope`, `group`, `resetsAt`, `retryAfterSeconds`, `usedBy` — sửa schema sẽ đổi `openapi.yml` và client sinh ra.
...
- [x] (phase 4) `IllegalStateException("CHAT_…")` + allowlist dò cause → exception có kiểu.

## 5. Hạ tầng triển khai

- [x] (phase 4) `infrastructure/deployment/deploy.sh`: ~20 assertion `[[ ]]` fail im lặng, `set -E` không có `trap … ERR`. Đề xuất thêm trap in dòng và lệnh lỗi. Chạm bề mặt deploy nên chờ bạn duyệt.

## 6. Phát sinh trong lúc sửa

- [x] (phase 3: một sync engine, lỗi của một item được ghi và run đi tiếp như Onyx) **Storage failure: Google và SharePoint xử lý khác nhau.** Google đánh dấu một node lỗi rồi đi tiếp; SharePoint retry cả attempt (đã sửa để không mất checkpoint, nhưng vẫn khác Google). Thống nhất theo bên nào?
- [x] **`VoiceSynthesisService.HttpSpeech`** vẫn tạo `HttpClient` mỗi lần vì `close()` gọi `shutdownNow()` để huỷ giọng đọc đang phát. Muốn dùng client chung thì phải huỷ theo request (`CompletableFuture.cancel`). Làm 2026-09-25 (phase 3): `HttpAudioStream` gửi bằng `sendAsync`, đóng stream thì huỷ đúng request đang chờ header hoặc đang đọc body; mọi giọng đọc REST dùng chung một `HttpClient`. Test: `HttpAudioStreamTest`.
...
- [x] **Bytes của recording có thể sót nếu tiến trình chết giữa hai bước** (CodeRabbit PR #363). `failAudio`/`failAbandonedAudio` commit FAILED trước, rồi mới `retire` upload `MEETING_AUDIO`. Nếu process dừng ở giữa, upload vẫn `ADOPTED` và `cleanupAbandoned()` không chọn nó. Khoảng hở này có sẵn trên main ở đường thất bại thường và đường ghi xong (`MeetingRecordingService` :127, :156, :190); sweep mới chỉ đi theo đúng mẫu đó. Đề xuất: marker cleanup bền vững hoặc sweep định kỳ các upload `ADOPTED` của meeting đã FAILED/đã có transcript — là thay đổi thiết kế nhỏ của object storage + meeting. Làm 2026-09-25 (phase 3): hàng meeting là marker bền — mỗi lượt của job recording quét các meeting `DONE`/`FAILED` còn giữ upload (index V129), retire rồi mới forget đúng upload đó; delete retire trước khi xoá hàng. Test: `MeetingRecordingRetirementTest`, xem [meeting spec](../../../specs/meeting.md#uploading-a-recording).
