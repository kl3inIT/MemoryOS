...
- [ ] **Provisioning catalog/persona của Chat.** Hiện `INSERT … ON CONFLICT` chạy lười ở ~20 entry point, kể cả mọi lần gửi tin và mọi endpoint đọc (nên không endpoint catalog nào `readOnly`). Đề xuất: provision khi tạo Tenant — cần event Tenant-created và cạnh phụ thuộc chat→IAM mới (conventions.md bắt quyết định kiến trúc). Phương án nhỏ: cache "đã khởi tạo" theo tenant trong bộ nhớ và đánh dấu các method đọc `readOnly`.
- [ ] **Provider OpenAI (~1.600 dòng) nằm trong `api`.** Quy ước: api/worker chỉ là composition root. Hệ quả: minutes và transcription của meeting buộc chạy trên API. Đề xuất: chuyển về `core/chat` (provider package) hoặc bundle `connector`; hoặc ghi nhận ngoại lệ có chủ ý trong ARCHITECTURE.md. Cùng loại: `GoogleDriveAccountClient` và `GoogleDriveOAuthProperties` (đọc tay `memoryos.google-drive.*` trùng `GoogleDriveProviderProperties`), logic admission trong `ActorSessionLoginSuccessHandler`, orchestration trong `MeetingController.minutes/publish`.
- [ ] **Interceptor CSRF + `throwOnError` toàn cục cho web client.** ~285 call site tự thêm `headers: sameOriginMutationHeaders, throwOnError: true`; quên là lỗi bị nuốt im lặng (đã gặp ở MCP). Đề xuất: một `client.interceptors.request.use` thêm `X-MemoryOS-CSRF` cho non-GET và `throwOnError: true` trong `client.setConfig`; trước đó sửa 3 caller tự đọc `result.error` (`chat-file-picker.tsx:198`, `chat-projects-page.tsx:217`, `search-page.tsx:188`). Đổi lại: mất marker tường minh ở từng call.
