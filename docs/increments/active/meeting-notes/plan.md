...
- [ ] **Meeting transcription adapter.** `MeetingTranscriber` port with an adapter per existing voice connection (`OPENAI`, `OPENAI_COMPATIBLE`, `ELEVENLABS`, `AZURE`), each declaring whether it diarizes; the administrator picks the connection used for meetings.
- [ ] **Soniox provider.** `VoiceProvider.SONIOX` (STT only), migration widening `ck_chat_voice_connection_provider`, realtime adapter with diarization and `context` terms, `/admin/voice` card; confirm the form accepts a provider without TTS.
- [ ] **Meeting module.** `io.memoryos.meeting` with `meeting`, `meeting_speaker`, `meeting_utterance`, `meeting_note` tables (next free Flyway version); owner-only application service; REST for list, create, read, rename speaker, edit notes; Tenant setting (all / administrators / off).
- [ ] **Streaming.** Meeting ticket bound to one meeting; WebSocket handler per track; relay to the provider; persist finalized utterances; reconnect with buffered audio; resume after a closed tab; usage in seconds per track.
- [ ] **Browser capture.** Microphone + `getDisplayMedia` tab audio as two PCM16 tracks through the existing worklet; per-track level meters; silent-tab and stopped-share warnings; unsupported-browser notice with microphone-only fallback; `beforeunload` confirmation.
...
- [ ] **Web UI.** Sidebar entry `Cuộc họp`; list grouped by date with status; new-meeting dialog with consent; meeting page with recording bar, live transcript, private notes, speaker naming and stop confirmation; result tabs (summary, decisions, action items, transcript) (follow the mock).
