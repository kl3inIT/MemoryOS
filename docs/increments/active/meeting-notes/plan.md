...
- [ ] **Share a meeting.** The participants field picks real members and Groups beside free-text names for people outside MemoryOS; `meeting_user_share` and `meeting_group_share` follow the shape used by Agents and Document Sets. A reader opens the meeting, watches the transcript arrive, reads the minutes and downloads the biên bản; only the owner edits or deletes.
- [ ] **Publish to the library.** Render the finished meeting as Markdown and publish it as a `chat_user_file`; republish on speaker, notes or minutes changes; *Open in Chat* attaches it to a new conversation. Readers reach it under the sharing above.
