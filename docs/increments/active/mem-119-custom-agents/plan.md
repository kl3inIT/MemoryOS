...
One pull request on `kl3inIT/mem-119-custom-agents`, committed in compiling slices. Onyx reference: `D:\MemoryOS\.tmp\onyx` (not in this worktree). `main` added V71–V73, so this increment uses V74; renumber again if `main` adds V74 first.

## 1. Authority and schema

- [x] V74: persona columns, owner Group (`ON DELETE SET NULL`), shares, labels, tools, MCP attachments, pins, preferences, prompt shortcuts; backfills; capability check constraint; `authorization_version` bump.
