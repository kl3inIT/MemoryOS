...
- **Sidebar.** Owner decision 2026-09-26: the application sidebar moves to the shadcn `sidebar` (persisted collapse state, `Ctrl/Cmd+B`, Radix tooltips when collapsed, `useSidebar`), restyled in `components/ui/sidebar.tsx` to keep the Onyx/Opal look of MEM-14 (15rem/4rem, the folded logo as the expand control, pinned footer, mobile drawer). The hand-written `SidebarTab`/`SidebarSection` go once nothing uses them; navigation stays driven by `admin-pages.ts`.
