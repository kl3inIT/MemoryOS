...
- `PostgresSourceConcurrencyTest` passes concurrent source creation, duplicate-byte convergence, composite tenant-FK rejection, PostgreSQL `SKIP LOCKED` lease reclaim, stale-token rejection, and inactive-Organization cancellation without publication.
- `TikaSourceContentExtractorTest` passes TXT, Markdown, PDF, DOCX, unsupported, encrypted, malformed, and write-limit behavior with misleading filenames and no provider type leakage.
- `SourceApiIntegrationTest` passes OWNER lifecycle, MEMBER denial, request-body and MVC method validation, malformed JSON preservation, bounded multipart rejection, immediate PUBLIC-access invalidation, remove/delete cleanup, and lost-response delete retry.
- `WorkerFileProcessingIntegrationTest` runs the actual scheduled worker with PostgreSQL and Tika through create → upload → ACTIVE → remove → Organization deactivation → successful cleanup. `WorkerApplicationSmokeTest` verifies composition with scheduling disabled.
- `OpenApiContractTest` passes against the regenerated Sources/SourceOperations contract; the Hey API client drift gate passes.
- `pnpm check` passes lint, formatting, TypeScript, 25 unit tests, route stability, and production build. Playwright passes 15/15 Chromium scenarios including the complete FILE Sources administration flow.
- `gradlew.bat clean check --no-daemon` passes 23 tasks. Final API and worker Docker targets build successfully, and `docker compose config --quiet` accepts the production API → worker health dependency and resource bounds.
- JetBrains inspected the changed IDE-supported set with warnings enabled before final cleanup. Final reinspection became unavailable because the MCP endpoint repeatedly timed out at its fixed 30-second ceiling; checked-in Gradle compilation, focused tests, full gates, generated drift checks, and image builds are the fallback evidence. Retained IDE warnings are the existing custom `X-MemoryOS-CSRF` header, Gradle's incubating repository mode, the conventional public Java launcher, and SQL files without an attached IDE datasource.
- No staging deployment was performed from the unreviewed local branch; exact-SHA deployment evidence remains pending PR review and merge.
