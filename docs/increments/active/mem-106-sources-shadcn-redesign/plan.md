...
- [ ] Large Drive trees. Each branch of the selection tree loads 25 items a page behind Load more, so nothing fetches a whole folder, but loaded rows accumulate and a single file among tens of thousands is reachable only through the search list (name and kind filters). Unsupported and unavailable items are labelled on their rows. A filter such as "unsupported only" needs the item status stored at discovery; it is computed from the Drive listing on each request today.
