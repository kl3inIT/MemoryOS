...
- [x] Bổ sung Search composer actions: `+` chỉ cho global `SOURCES_MANAGE` và deep-link tới FILE Source setup; voice input dùng browser Web Speech API, nối transcript vào query mà không tự submit hoặc gửi audio lên server; có recording/unsupported/permission/error feedback, cleanup và accessible pressed/disabled state.
