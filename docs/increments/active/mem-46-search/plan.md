...
- [x] Search workspace riêng theo reference Onyx: empty state căn giữa brand/câu dẫn/query, sau submit chuyển sang toolbar, result feed và desktop file-type facets; loading/empty/unavailable/cancel/out-of-order responses, escaped snippets/highlights, filter/pagination và source opening. Default Search không gọi generative ChatModel.
