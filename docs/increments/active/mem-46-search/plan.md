...
- [x] Search workspace riêng theo reference Onyx: empty state căn giữa brand/câu dẫn/query, FLIP dock transition có reduced-motion fallback, compact Radix time/file-type menus, rồi result feed và desktop file-type facets; loading/empty/unavailable/cancel/out-of-order responses, escaped snippets/highlights, filter/pagination và source opening. Default Search không gọi generative ChatModel.
- [x] Bổ sung Search composer actions: `+` mở Radix menu chỉ phục vụ Search, cho global `SOURCES_MANAGE` upload FILE Source thật và global/scoped `SOURCES_READ` xem nguồn đã kết nối; không có Chat action/one-off attachment giả, có keyboard/Escape/focus return. Voice input dùng browser Web Speech API, nối transcript vào query mà không tự submit hoặc gửi audio lên server; có recording/unsupported/permission/error feedback, cleanup và accessible pressed/disabled state.
