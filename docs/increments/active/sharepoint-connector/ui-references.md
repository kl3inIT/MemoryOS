...
## 8. Nghiên cứu bổ sung ngày 18/09/2026

Đợt rà soát lại giữ nguyên Onyx làm baseline hành vi, nhưng so sánh thêm các pattern enterprise mới:

- [LangChain integrations](https://mobbin.com/screens/c3ee2df7-4268-4daa-a45d-194770781ec6) nhóm connector theo hãng, giữ trạng thái quyền ngay trên hàng; [Base44](https://mobbin.com/screens/0b200b54-e133-4f16-a4bc-8da1cdec4c0d) tách Connected và Available trong cùng bề mặt. MemoryOS giữ catalog nhỏ hiện tại, không thêm category rail, nhưng trạng thái và hành động phải đọc được ngay.
- [Pinterest data-source setup](https://mobbin.com/screens/b87c054f-7fe4-4d30-b353-2ff4f4a67c7b) dùng rail bước cố định và footer điều hướng ổn định; [Vanta AWS setup](https://mobbin.com/screens/ac07a889-bd30-4ff0-b081-1f62cc8da946) đặt giải thích quyền cạnh control thay vì dồn vào tooltip. Luồng SharePoint giữ bốn bước Credential → Content → Access → Review, mở rộng chiều ngang để phần giải thích và form không bị nén.
- [Okta connector authentication](https://mobbin.com/screens/6ec96141-0055-455f-a19d-4d1f2a3036dd) tách cấu hình kỹ thuật và preview; [Cohere connector](https://mobbin.com/screens/261a73ad-2740-481d-a528-96a721c8fec6) nhóm authentication thành một vùng rõ. Hộp thoại credential của MemoryOS dùng `Dialog` shadcn, chia hướng dẫn Entra và credential thành các vùng có tiêu đề thay vì một form dài không phân cấp.
- Glean đặt SharePoint dưới Admin Console → Data sources, xác minh credential/quyền trước khi crawl, tách initial crawl khỏi steady-state metrics và giữ hành động xử lý lỗi ngay trên data source. MemoryOS giữ contract hiện có: tạo trả receipt, xác minh scope nền, trạng thái sync/prune và lỗi có hướng xử lý trên trang chi tiết; không sao chép bộ quyền FullControl của Glean vì MemoryOS không đồng bộ ACL trong increment này.

Điều chỉnh trình bày:

- Header setup dùng action “Exit setup”; rail bước nằm trong một bề mặt riêng, sticky trên desktop.
- Chọn phạm vi và visibility dùng radio-card có mô tả/hệ quả quyền ngay trong từng lựa chọn.
- Review dùng card theo từng bước, mỗi card có hành động Sửa quay đúng bước; lịch đồng bộ vẫn là cấu hình vận hành riêng.
- Trang chi tiết nhóm Synchronization, Credential, Saved scope và Schedule thành các card độc lập; cảnh báo nằm trước vùng bị ảnh hưởng.
- Không thêm abstraction UI mới: dùng `Card`, `Dialog`, `RadioGroup`, `StatusBadge`, `Collapsible`, `Table`, `Switch`, `Button` và `IconButton` đã có.
