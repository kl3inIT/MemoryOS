...
- **Release pipeline.** `deploy-staging.sh` accepts exactly the current image set (`images.env` has four lines). Adding the interpreter to staging therefore changes the release contract, CI publication and the deploy script together. That work is phase 1, not phase 0.
