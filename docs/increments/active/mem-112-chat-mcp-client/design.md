...
A further check settled the question. `DynamicClientRegistrationService` obtains authorization-server metadata through Spring Security's `ClientRegistrations.fromIssuerLocation`, which probes OIDC path insertion, OIDC path appending, then RFC 8414 path insertion. MCP requires the reverse priority — RFC 8414 insertion first — with `MUST`, and also requires refusing an authorization server whose metadata omits `code_challenge_methods_supported`, which Spring does not check. Adopting the library would therefore make MemoryOS non-conformant on a `MUST`. Revisit if it gains a Tenant-aware registration store and spec-ordered authorization-server discovery. Separately, Spring AI does not instrument MCP client tool calls ([spring-ai#4560](https://github.com/spring-projects/spring-ai/issues/4560) concerns server-side `@McpTool`), so the `memoryos.chat.mcp.call` timer duplicates no framework instrumentation.
