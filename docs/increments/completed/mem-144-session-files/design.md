...
2. **A conversation's files are its artifacts plus the uploads attached in it.** Artifacts carry `session_id` (V89). An upload carries no conversation, so the upload branch matches through the conversation's own messages: `chat_message.files` holds the descriptors, and the filter reads them for that one session, which needs no index because it is scoped to a single session's rows before the JSONB is expanded.
