...
### Whole-document PDF view by HTTP range (owner-approved 2026-09-14)

Staging originals are 18–19 MB, so the first cited-page version downloaded the whole file before showing five pages. The owner asked to read the full file. RAGFlow reads whole objects into memory and its frontend downloads them twice; Onyx links out; Box Content Preview and Firefox's viewer use pdf.js range loading. Defaults chosen with the owner: load on demand (`disableAutoFetch`) and keep `Cache-Control: no-store`.

- `ObjectStorage.openRange(key, first, last)` (S3 `GetObject.range`, provider `Content-Range` parsed for the whole size). Spring's `InputStreamResource` range support would re-read from byte 0, so ranges are parsed in `DocumentOriginalResponses`.
- `DocumentOriginalService` takes an optional `ByteRange`: the full authority path runs for every range; ranged reads inspect metadata first (no whole-object checksum on a range) and check `%PDF-` when starting at 0. 206/416 and `Accept-Ranges` as in the [Search spec](../../../specs/search.md).
- `DocumentPdfView` passes `{ url }` with `disableStream`, `disableAutoFetch` and a 1 MiB `rangeChunkSize`; pdf.js keeps whole reads for files up to 2 MiB. Every page has a sized placeholder (`pageHeight`); `pagesToRender` mounts the cited page plus pages within one viewport of the visible area and one neighbour. Two `IntersectionObserver`s track near pages and the current page. The view scrolls to the cited box once the page is measured; zoom keeps the current page.
- If range loading fails, the view reloads the original once with `disableRange` (the former whole read) and reports the failure to Sentry as workflow `pdf-view`, stage `range-load` or `whole-load`. A 9 MB staging original failed to open by range on 2026-09-14 while server ranges were correct and five local PDFs (4–22 MB, CSP and fast cancelled first response reproduced) opened; the report names the pdf.js error for that file.
- pdf.js image decoders (JBIG2, JPEG 2000) are emitted from `pdfjs-dist/wasm` to `assets/pdfjs-<version>/` and passed as `wasmUrl`. The CSP forbids WebAssembly, so pdf.js uses their JavaScript fallbacks; without `wasmUrl` scanned pages rendered without their images.
- Known limits: originals up to 2 MiB download whole (pdf.js threshold); suffix and multiple ranges get the whole object; pdf.js fetches the URL itself, so an expired session shows the PDF error rather than the login redirect of SDK calls. pdf.js cancels its first whole-file response after the headers: the API then closes the S3 stream, and the JDK URL connection drains at most 512 KB before dropping the socket, so the abort stays bounded. The nginx rate-limit zones key only invitation requests, so range bursts are not limited.
- Linearizing originals (qpdf) is only a follow-up if staging measurements stay slow.
...
- `EvidenceViewSwitch`: Radix `Tabs` segmented list "Đoạn trích | Trang PDF" (tablist semantics, arrow keys) shown only for PDF evidence with recorded pages; inactive panels unmount, so pdf.js loads only on the PDF tab. `DocumentPdfView` is lazy-loaded (pdf.js stays out of the main bundle and unit tests), renders the whole document by range (see above), falling back to one whole read when range loading fails, without text/annotation layers on a sunken canvas with a current `Trang x / n` counter and fit/150/200/300% zoom (a Letter page at Chat panel width is about 0.6×, too small for body text), highlights boxes and scrolls to the first. Boxes use `mix-blend-multiply` over the always-white page so cited text stays black (a translucent fill greyed it); that highlighter token is theme-independent, while passage highlights use per-theme `evidence-highlight` tokens. The worker is bundled from `pdfjs-dist` (`script-src 'self'`).
