...
- Kept: collection during SOURCE_SYNC, V75/V76 persistence, `GoogleDriveAclChanged`, the consumer read `GoogleDriveAclReader.readByDocument`, and the internal per-file `JdbcGoogleDriveAclRepository.read` used by synchronization tests. Both reads now share one snapshot projection.
