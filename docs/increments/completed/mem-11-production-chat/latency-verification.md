# Phase 3.3 — reference implementation retrieval parity and latency

Implementation and verification on 2026-09-10, in one PR. Reference checkout: reference implementation `40eb240df370688ed6eeedc1272116e7829554ac`; applicable retrieval files are unchanged from the accepted `bd89d269` baseline. The current contracts live in [Chat](../../../specs/chat.md), [Search](../../../specs/search.md), and [model options](../../../specs/chat-models.md).
...
- Native `CompletableFuture.cancel(true)` does not interrupt provider work. The native `ExecutorAsyncer` therefore submits through a scope that tracks actual native completion, including its usage recording. Cleanup waits at most one second by default. If a provider ignores interrupts, terminal usage remains unknown and the native process, client lease and execution slot are retained until actual completion; late work cannot publish results or retry provider IO. Native binding/retries/accounting remain in Embabel.
...
## Review follow-up — PR #91, 2026-09-10

Captured CodeRabbit review: 11 findings on `d7e2d33`; the existing `61be4b1` follow-up only added corpus test/measurement evidence. Ten findings are addressed in this fix round. The generic adapter keeps explicitly configured reasoning minima: GPT-5 mini uses the already verified `minimal` default; a global ban on `none` would reject models that support it. An added real SDK HTTP capture case verifies omitted helper options become `minimal` while answer reasoning stays `high`.

The cancellation fix bounds cleanup to a one-second default grace (maximum five seconds). Native process/client/admission ownership lasts until actual work ends, even after a terminal outcome with unknown usage. The regression suite deliberately blocks provider bodies while ignoring interrupts, verifies bounded Stop and rejects late retries, evidence and terminal accounting updates. This does not prove that a remote provider stops billing after cancellation.

Focused verification passed in 1m33s: 59 passed, three opt-in live-provider tests skipped. This includes native API execution and real PostgreSQL/HTTP fixtures, startup-latch timeout tests, client/capacity retirement, long progress titles and mixed malformed author metadata. The SDK clients and owned registry are closed even if receipt writing fails; corpus path/content are Gradle inputs and live measurement mode bypasses task/build-cache reuse.

Every edited Java/Kotlin DSL file was inspected with warnings enabled. Retained weak warnings are the existing test-container HTTP URL and fixture SSE/custom headers. Explicit shutdownNow is documented because ExecutorService.close would wait indefinitely. Gitleaks found no leaks in the review diff. The final repository `gradlew.bat clean check --no-daemon --console=plain` passed in 8m34s: 556 passed, 6 opt-in checks skipped, zero failures/errors. This fix did not rerun the live corpus benchmark; the dated measurements above remain the earlier receipts.
