...
Source review: 2026-09-08. No application dependencies or runtime code changed; upstream samples were not built or executed. This review evaluates assistant-ui, MemoryOS-owned Chat history and background execution, Embabel's model/tool loop, Spring AI model integration, and the existing Search capability. Earlier fixed-pipeline research was superseded and has been removed.
...
`embabel-state`, `embabel-hitl`, `ask-user-question`, skills and subagent examples are secondary references if the product scope requires those behaviors. HyDE, semantic caching, autonomous coding tools and long-term auto-memory are not additions to the initial reference implementation-aligned knowledge Chat. `chatclient-and-react` demonstrates ReAct reasoning, not a React browser adapter.
...
1. The design/plan reconciliation is now recorded in [the current design](design.md) and the retained [retrieval diagram](diagrams/README.md).
...
3. Adapt recipe patterns for the authorized Search tools, follow-up query plan and validated section selection, preserving the reference implementation retrieval baseline.
...
5. Verify real-model behavior, permissions, failure recovery and agreed reference implementation functional parity. Recipe context-load tests are not production acceptance.
