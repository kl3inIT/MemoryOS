...
Linear: [MEM-192](https://linear.app/memory-os/issue/MEM-192). Liên quan: [MEM-171](../mem-171-production-deployment/design.md) (production), [MEM-135](../../completed/mem-135-embedding-settings/design.md) (embedding trên cùng GPU), [MEM-193](https://linear.app/memory-os/issue/MEM-193) (model chat trên cùng GPU), [MEM-191](../mem-191-extraction-fallback/design.md) (rơi về bộ đọc gốc), [MEM-79](../../superseded/mem-79-rancher-ocr/design.md) (service OCR trên cụm dev).
...
MemoryOS cần bbox: [document viewer](../../active/document-viewer/design.md) tô sáng trích dẫn PDF bằng `page_no` + `bbox`, và [Document contract](../../../specs/document.md) liệt kê `prov.bbox` là khoá mà Chat và Search đọc.
...
**Chunker.** Ở chế độ có nhãn, ô tiêu đề trống không thêm nhãn và ô trống không thêm dòng, như chế độ địa chỉ; cột không có tiêu đề đọc `Column N`. Dòng tiền của bảng cân đối đọc `TÀI SẢN: 1. Tiền`, `Mã\nSố: 111`, `Số cuối kỳ: 2.470.482.178.999`, `Số đầu kỳ: 2.764.761.087.606` (`\n` là chữ Paddle trả về). `DocumentChunk.CONVENTION` lên `structured-cl100k-768-v4`, vì bảng Docling có ô trống bớt dòng `Nhãn: `; giữ v3 thì một index trộn chunk cũ với chunk mới. Mỗi môi trường tự dựng lại từ artifact đã lưu (MEM-135). Tiêu đề cột và tên bảng của PaddleOCR-VL nằm trong artifact, nên bản scan đã đọc trước thay đổi này chỉ có chúng sau khi Source được đánh chỉ mục lại.

**Kiểm tra bảng tài chính.** `FinancialTableDiagnostics` đọc `columnHeader`, nên đã chạy trên cả 156 bảng với và không có tiêu đề: 153 bảng cho kết quả y hệt; hai báo cáo lưu chuyển tiền tệ vẫn `INCOMPLETE` với ít lỗi `MISSING_PERIOD_HEADERS` hơn, và một (6 tháng, trang 13) từ `INCOMPLETE` thành `CONSISTENT` cho kỳ này. Không bảng nào xấu đi.
