...
- **Hot paths.** Search index ensured once per generation per process (re-ensured on a 404 write); no alias HEAD per search; row-value reconcile cursor; bulk projection maintenance; gateway failures logged. Chat send loads the persona (with its tools, MCP servers and Sources) and capabilities once and carries them through model selection into the reservation, which share-locks the persona and rechecks only its revision and the owner's use authority; a Document Set's own Source list and the conversation's pinned reasoning level are not part of that revision, so a change made in the milliseconds between the read and the reservation applies from the next turn. Context files are read in bulk; branch copy and persona reorder in bulk; the turn finish reads its outcome from the terminal UPDATE. SharePoint resolves its scope once per run; fewer lock queries per item; Google Drive membership reuses the run's own rows.
