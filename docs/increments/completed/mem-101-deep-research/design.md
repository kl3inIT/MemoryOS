...
- No survival of execution across API restart. Onyx loses in-flight research on restart as well. Reload, tab close and client disconnect are covered by the background run and stream replay; since [MEM-26](../../active/mem-26-chat-stream-redis/design.md) the replay lives in Redis and survives the restart, and the browser resumes until the outcome instead of polling history.
