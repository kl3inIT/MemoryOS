...
`/admin` is a full-width semantic configured-Source table and contains no selected detail pane. Its stacked icon/title header and right-aligned action lead into a compact search/collapse/filter toolbar. Sources are grouped by provider under expandable six-column summary rows that expose total, active, and public Source counts plus total indexed documents. Expanded groups reuse those tracks for Name, Last indexed, Status, Permissions / Access, Total docs, and an icon-only Manage action; Source links navigate to `/admin/sources/{sourceId}` for uploads, indexing, cleanup, and destructive actions. `/admin/sources/new` lists only implemented Source types. Selecting FILE opens `/admin/sources/new/file`; successful creation returns to the dedicated detail route for the created Source.
