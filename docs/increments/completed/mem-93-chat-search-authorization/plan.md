...
- Moved out of scope: Google Drive per-file tokens (`google_user:`/`google_group:`/`google_domain:`) wait for the MEM-88 identity contract and are tracked there, not by this increment. [MEM-105](../mem-105-source-access-modes/design.md) now derives `google_user:` and `google_domain:` tokens; `google_group:` remains unimplemented.
