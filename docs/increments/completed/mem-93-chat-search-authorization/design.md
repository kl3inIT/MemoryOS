...
- **Legacy chunks and backfill.** Chunks written before the access fields existed match the filter until V52's `ACCESS` backfill rewrites them; the post-query database recheck authorizes every hit meanwhile. An `ACCESS` operation rewrites `source_metadata`, `metadata_hash` and the access fields with per-chunk `_bulk` partial updates by chunk ID (no scroll permission, as in the reference); it never marks the document pending, never embeds, and retries briefly when the document is waiting for a content rewrite (which reads access itself).
