...
> - Staging host (manual, before the deploy that needs it): create `/apps/memoryos/secrets/redis/api-password.txt` (mode 0600, as `provision-staging-secrets.sh`), add `MEMORYOS_REDIS_API_PASSWORD_FILE`…
> - `clean check` (CI is the gate if the host runs out of memory), push, PR, CI green, merge.
> - Staging acceptance: a Deep research turn streams progress to the end without "Đang kết nối lại"; reload mid-run replays from Redis; restart the API mid-turn and observe the reply end `CHAT_INTERRUP…
...
- [x] API configuration: `spring.data.redis` from `MEMORYOS_REDIS_*`, staging SSL bundle, Redis health indicator off for the API, one API-owned Arconia Redis dev service on `56379` with the worker configured only as its client, Redis Testcontainer in tests. Arconia 0.30 Redis does not support Dev Service discovery; removing the worker's provider prevents the two JVMs from racing for the fixed port.
- [x] Development Redis ownership regression verified (2026-09-17): `WorkerDevelopmentConfigurationTest` passes; `:worker:check` passes; with the API already `UP`, the real worker starts with the `development` profile, reports readiness `UP`, and uses the single API-owned `redis:8.8-alpine` container on `56379` without starting Testcontainers.
