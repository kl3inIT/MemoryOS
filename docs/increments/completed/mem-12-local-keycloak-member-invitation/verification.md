...
The final combined owner-to-recipient acceptance and exact-SHA closure are recorded below. Public-mail delivery remains outside scope: Mailpit is intentionally staging-only and captures rather than relays.

## Final Organization-only staging evidence

On 2026-08-26, PR #32 reviewed head `90eac2ec8beb3a2448aa8145e09d9560e3f3781f` merged as `c51dd088f670a8d92808e445c4e410c5c8c18c36`; PR #33 fixed explicit self-hosted Infisical domain propagation and merged as `a17ea6c88e647fdd6c2f638dc26cf65061d23b48`. Exact merge-SHA main CI run `32978942562` passed backend, frontend image, frontend, and 12 Playwright scenarios.

Before staging cutover, the API writer stopped and the ops backup profile created `memoryos-20260826T135912Z.dump`, `keycloak-20260826T135912Z.dump`, restore lists, and manifest `20260826T135912Z.sha256`. Container and off-host SHA-256 checks matched. A restored `memoryos_mem28_rehearsal` database preserved the `1 actor / 1 binding / 1 Organization / 1 membership / 1 pending invitation` baseline; the cutover image applied V4, became healthy, preserved those counts, and removed every Workspace table and column.

The staging runtime then deployed API and web images tagged `sha-a17ea6c88e647fdd6c2f638dc26cf65061d23b48`. A real owner impersonation projected `OWNER` plus `INVITATIONS_MANAGE`. The owner created one temporary invitation; a temporary enabled, verified Keycloak user authenticated with the invited email, atomically received one Organization `MEMBER` row, projected `MEMBER` with no capabilities, and received `403 INVITATION_NOT_OWNER` from the owner-only invitation API. The temporary user, invitation, binding, Actor, membership, and application sessions were deleted; counts returned to `1/1/1/1/1/0` for Actor/binding/Organization/membership/invitation/session, preserving the original pending invitation.
