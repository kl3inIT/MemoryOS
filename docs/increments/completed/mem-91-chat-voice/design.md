...
**`chat_voice_connection`** — V82, Chat sở hữu, JPA `VoiceConnectionEntity`:

| Cột                                   | Onyx                                      | Ghi chú                                                                                                                                                                                                                                   |
| ------------------------------------- | ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `id` UUID, `tenant_id`                | `id`                                      | Theo Tenant [MemoryOS]                                                                                                                                                                                                                    |
| `provider`                            | `provider_type`                           | Enum `VoiceProvider`. Hiện có `OPENAI`, `OPENAI_COMPATIBLE`, `ELEVENLABS`, `AZURE`; CHECK có tên `ck_chat_voice_connection_provider` và chỉ được nới khi adapter của provider hoàn thành. `UNIQUE (tenant_id, provider)`, vì Onyx thực tế cũng mỗi loại một dòng |
...
**`chat_voice_settings`** — V83, JDBC `JdbcVoiceSettingsRepository`:
