...
> - Confirm Drive MCP access for the owner's account and record its user type and publishing status (`Internal` needs a Workspace organization; `External` + Testing limits test users to 100 and expires…
...
> - Session-bound encrypted Keycloak token retention removed at sign-out; Keycloak token exchange (RFC 8693) to the MCP server audience per call, as OrgMemory `apps/mcp` does, instead of forwarding the…
