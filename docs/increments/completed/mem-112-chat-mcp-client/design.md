...
- Evidence to choose: owner accepts retaining login tokens; interaction with [logout-without-keycloak-page](../../active/logout-without-keycloak-page/design.md).
...
`DynamicClientRegistrationService` obtains authorization-server metadata through Spring Security's `ClientRegistrations.fromIssuerLocation`, which probes OIDC path insertion, OIDC path appending, then RFC 8414 path insertion, where MCP asks for RFC 8414 insertion first. That ordering difference is weak grounds on its own: it changes the outcome only for an authorization server that publishes different metadata at more than one of those URLs. The decisive reason is the storage model above. The missing `S256` check, the followed redirects and the unbounded metadata bodies are costs that could be added on top of the library, not blockers. Revisit if it gains a Tenant-aware registration store.

**Conformance is a means here, not an end.** This increment already departs from the specification where the accepted MemoryOS policy says otherwise: MCP requires every authorization-server endpoint to be HTTPS, while the endpoint policy above permits internal HTTP. A `MUST` in the specification is evidence about interoperability, not an argument that ends a design discussion. The live counter-test to watch is the `resource` parameter, which MCP requires clients to send to every authorization server: if a real provider rejects it, observed behaviour wins and the departure is recorded here. Separately, Spring AI does not instrument MCP client tool calls ([spring-ai#4560](https://github.com/spring-projects/spring-ai/issues/4560) concerns server-side `@McpTool`), so the `memoryos.chat.mcp.call` timer duplicates no framework instrumentation.
