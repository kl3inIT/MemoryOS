...
6. **Rendering.** The browser renders `line`, `scatter` (points without a line), `bar` and `pie` with the repo's shadcn chart component (Recharts), in a card below the answer with an Interactive/Static toggle; `superchart` renders its subcharts; `box_and_whisker` (Recharts has no box plot), `unknown`, invalid or oversized data, and a superchart with any such subplot fall back to the PNG. Series use a categorical palette, because the neutral `--chart-*` tokens cannot tell series apart. The PNG stays the download.
