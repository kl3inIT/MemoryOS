...
> Superseded 2026-09-27 by owner decision: production OCR runs through [MEM-192](../../completed/mem-192-ocr-gpu/design.md) (PaddleOCR-VL on the serving GPU, Docling for text layers) on the Compose nodes of [MEM-171](../../completed/mem-171-production-deployment/design.md); this Rancher deployment was not taken further.
...
One conversion worker, offline model assets, 100 MiB input, 200 pages, and a
3600-second document budget with 3610-second synchronous wait bound the service.
...
ingress to port 5001. Its 150 MiB body allowance accounts for base64 JSON encoding;
3630-second proxy timeouts cover the service's synchronous wait. The direct
...
The standalone Kubernetes Deployment owns these server settings; the caller's
Infisical environment does not inject values into this pod. The 100 MiB,
sixty-minute limit correction retains the image, resources, authentication and
OCR options. Uvicorn graceful shutdown is 3620 seconds, inside the pod's
3630-second termination grace period. Apply the Ingress and Deployment together,
then verify the file/timeout settings inside the replacement container and
perform an authenticated 100 MiB scanned-PDF conversion through ingress.
Do not change the already-confirmed Infisical values or automatically reindex
user documents. File admission does not establish full-corpus resource capacity.
