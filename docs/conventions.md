...
- **Lint target.** `pnpm lint:target` runs `.oxlintrc.target.json`: the base rules plus `jsx-a11y`, `import` and `@shadcn/lint`, with named exceptions (brand marks, `aui-*` and `brand-loader` classes, dynamic geometry inline styles, `calc()`/`var()` sizing, the vendored primitives) and `no-restyle` contracts that let layout, padding and text placement through on tables, inputs, triggers and buttons. New and converted code passes it; it replaces `pnpm lint` once the sweep reaches zero.
