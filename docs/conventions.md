...
Document Sets (`web/src/features/document-sets`) is the reference implementation of each pattern below.

- **Data layer.** Queries and mutations spread the generated factories from `@/lib/hey-api/@tanstack/react-query.gen`: `useQuery({ ...listXOptions({ query }), select })` and `useMutation(xMutation())`. Invalidation uses the generated keys (`listXQueryKey()` matches every page of the list by prefix; `getXQueryKey({ path })` one item), gathered in one feature helper such as `invalidateDocumentSets`. Keys carry no actor or authorization version: `acceptCurrentIdentity` purges private queries when either changes. Where the published schema marks fields optional that the API always sends, one narrowing function over the generated type (`documentSetOf(view: View)`) replaces a hand-written response schema; request validation extends the generated zod schema from `@/lib/hey-api/zod.gen` (`zInput.required().extend(...)`). A picker that needs every item reads the complete list page by page under the key `[...listXQueryKey(), "all"]` (`agentDocumentSetsOptions`, the Search Document Set filter), so invalidating the list key refreshes it with the paged reads. Conversations are refreshed with `invalidateChatSessions` (`features/chat/chat-api.ts`), or `useRefreshChatSessions` where the assistant-ui thread list must reload too; the file library with `invalidateLibrary` (`features/library/library.ts`).
- **Secrets in requests.** A request that carries a secret (an API key, a client secret, a service-account key) is a direct SDK call or a mutation with `gcTime: 0`, never a query and never a mutation left in the cache: the query cache keeps keys, data and mutation variables in memory after the request ends, where the devtools, structural sharing and any persister can reach them, so a key sent once would outlive its request.
- **Data tables.** A table with paging, sorting or row selection builds a TanStack Table v9 instance in the page (`tableFeatures({ rowPaginationFeature, columnMeta: {} as DataTableColumnMeta })`, `createColumnHelper`, `useTable`) and renders it with `DataTable` from `@/components/data-table/data-table`, which draws the `Table` primitives in one frame. Paging and sorting stay on the server: `manualPagination`/`manualSorting` with the state owned by the page and part of the query options, `pageCount` or `rowCount` from the response (an offset API without a total requests one extra row), `placeholderData: keepPreviousData`, and `TablePagination` in the `footer` slot, disabled while `isPlaceholderData`. A cursor-paged API uses `useCursorPaging` with `cursorTablePaging` from `@/components/data-table/use-cursor-paging`, which walk back through the cursors already seen and drive the table with `table.nextPage()`/`table.previousPage()`. Columns are declared at module scope, because a cell renderer is a component and a rebuilt column list remounts every cell; a column shown only sometimes is hidden with `columnVisibilityFeature` and `state.columnVisibility` rather than a second column list, and `DataTable` leaves hidden columns out. `rowProps` sets each body row's click handler, `data-*` attributes and `selected` state (drawn selected and exposed as `aria-selected`); a clickable row is only a larger pointer target, and a control inside it stays the keyboard's way to the action. `empty` shows an `EmptyState` across the table when a page has no rows, with the pager still under it. Static tables stay plain `Table`; a bounded one takes `maxHeight` so its `sticky` header sticks to the box that scrolls.
- **Forms.** Forms use `useAppForm` from `@/components/form/app-form` (TanStack Form `createFormHook`) with a zod schema as the `onDynamic` validator and `validationLogic: revalidateLogic()`, so errors appear on submit and follow each change afterwards. Bound controls render through `form.AppField` with `field.TextField`/`field.CheckboxField`, which wire `FieldLabel`, `FieldDescription`, `FieldError`, `data-invalid` and `aria-invalid`; a custom control inside `form.AppField` reads `useFieldValidity()` from `@/components/form/form-context`. `form.AppForm` holds `form.SubmitButton` and `form.FormError`. A failed mutation in `onSubmit` calls `setServerErrors(formApi, problemErrors(error))` with `useProblemErrors()`, which places `ApiProblem` field violations on their fields and the problem on the form. Server errors live under the `onServer` key, never `onSubmit`: TanStack Form re-evaluates a key only through a validator of that key, so an `onSubmit` entry without an `onSubmit` validator was never cleared and the next submit stopped at validation without calling `onSubmit`. The built-in `onServer` validator clears the entry on each change, blur and submit, and `useAppForm` re-evaluates each field's entry on submit, so no form declares a placeholder validator or clears errors itself. A component that renders part of a form built elsewhere is declared with `withForm` and receives the form as a prop.
- **Unit tests.** HTTP is mocked with MSW, never by mocking the generated SDK module. `src/test/setup.ts` runs the server from `@/test/msw` and resets it after each test; a test declares responses with the generated `handle<Operation>` factories from `@/lib/hey-api/msw.gen` (`server.use(handleListDocumentSets({ body: [view] }))`) or a resolver that inspects `params` and `new URL(request.url).searchParams`. An unhandled request is logged and rejected.
- **Callout roles.** An `Alert` announces by its role: `alert` (the default) for a failure the person must act on now, `status` for progress and results, such as a successful connection test, and `note` for standing information that is not news, such as a capability that is not configured.
- **Notifications.** Action feedback stays on `ActionNotifications` (`ui/action-notifications`, Radix Toast) rather than the sonner toast the shadcn skill suggests for Radix projects: a notification belongs to the page that raised it and leaves with it unless it is raised with `surviveNavigation`, and its copy is typed i18n keys, both of which sonner's global queue and free strings would give up.
- **Pending buttons.** `Button`, `IconButton` and `TextButton` keep their `pending` prop, although the shadcn skill composes `<Spinner data-icon="inline-start" />` inside a disabled `Button` by hand: `pending` disables the control, sets `aria-busy` and draws the decorative `Spinner` (`IconButton` swaps its icon for it) in one place, so every busy action behaves and announces the same. Icons inside buttons carry `data-icon="inline-start"` or `"inline-end"` and no size classes; the button sizes them.
- **Lint.** `pnpm lint` runs `web/.oxlintrc.json` with `--deny-warnings`: `react`, `typescript`, `oxc`, `jsx-a11y` (without `prefer-tag-over-role`, by owner decision), `import` and `@shadcn/lint`. Arbitrary values that recur become theme tokens or `@utility` classes in `web/src/styles/theme.css` rather than exceptions. The named exceptions are brand marks, `aui-*` and `brand-loader` classes, dynamic geometry inline styles, `calc()`/`var()` sizing, the vendored primitives and their variant tables under `components/ui` (which define the token plumbing), and the upstream input-group click-to-focus addon; `no-restyle` contracts let layout, padding and text placement through on tables, inputs, triggers and buttons. A single-line exception carries an `oxlint-disable-next-line` comment with its reason. `pnpm check:knip` reports unused files, exports and dependencies outside the generated client and `components/ui`.
