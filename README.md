...
The merged layout contains 78 migrations. Published `main` V1–V74 remain unchanged; this integration assigns SharePoint to V75/V76 and Voice to V77/V78. Historical local V37–V42 are V43 Basic seeding, V44 system-grant renaming, V45 derived-only Group read, V46 unified Source management, V47 scoped manager ownership and V48 ordinary-only Source associations. Model-management grants are preserved; standalone read/delete grants are revoked without promotion. Exact current filenames are listed in [Architecture](ARCHITECTURE.md#data-ownership-and-consistency).
