...
API startup runs all 87 Flyway migrations through V87, transactionally bootstraps or verifies the configured Tenant UUID and owner, provisions protected Admin/Basic Groups with SYSTEM_ADMIN/SYSTEM_BASIC, and binds Arconia Web fixed Tenant context around HTTP requests. IAM lifecycle uses JPA; bounded projections, authorization locks, and Source/Document/Ingestion/Object Storage mechanics remain concrete JDBC persistence. Group grants and managed-Group scope authorize each request; Basic supplies direct Search access without Source administration. Source upload initiation returns a checksum-bound presigned PUT; finalization reauthorizes after provider inspection before adoption. Worker starts after migrated API health, carries each work record's explicit `TenantId` through fenced indexing/cleanup, and uses FILE/Docling or bounded Tika extraction. Chat runs inside the API on virtual threads with a single hosted O200K token estimator; no vendored tokenizer assets or native libraries ship in either image.

The integrated tokenizer backfill is V54, the Web search gateway constraint is V55, V61 narrows the persisted Web intent to off/auto, V75–V78 add Google Drive ACL snapshots, persisted run errors, Source access modes and the default Drive interval, V79 adds Source pause/resume, V80/V81 add SharePoint, V82/V83 add Voice, V84 adds model flows and the provider data boundary, V85 the AI usage ledger, V86 Chat preferences and V87 drops the Chat start page. Main V1–V53 are preserved. Do not start this layout against a local/review database carrying the former branch-only tokenizer V37. Preserve that runtime until a separately authorized data-preserving cutover; see [migration ownership](ARCHITECTURE.md#data-ownership-and-consistency).
...
The merged layout contains 87 migrations, V1–V87. Historical local V37–V42 are V43 Basic seeding, V44 system-grant renaming, V45 derived-only Group read, V46 unified Source management, V47 scoped manager ownership and V48 ordinary-only Source associations. Model-management grants are preserved; standalone read/delete grants are revoked without promotion. Exact current filenames are listed in [Architecture](ARCHITECTURE.md#data-ownership-and-consistency).
...
MCP servers (MEM-112, in progress) store their OAuth client secrets, header templates and User credentials sealed with `MEMORYOS_MCP_CREDENTIAL_ENCRYPTION_KEY` (base64-encoded random 32-byte AES key), supplied to the API only. Missing configuration disables MCP secret writes and reads. Preserve the key with database backups; changing it without re-sealing makes stored MCP credentials unreadable. MCP OAuth callbacks use `MEMORYOS_MCP_REDIRECT_URI` (HTTPS, or loopback HTTP for local development), which must end with `/login/oauth2/code/mcp` and be registered on each OAuth client. See the [MCP server runbook](docs/runbooks/mcp-servers.md) for outbound domains, the per-organization OAuth application and the Client ID Metadata Document's inbound requirement.
