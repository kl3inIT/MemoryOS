...
- [MEM-142 — My file library](docs/increments/active/mem-142-file-library/design.md) owns the owner-private `/library` page, the union listing over uploads, generated files and generated images, artifact deletion with durable byte release, and the named refusal for a file a Project or Agent holds; hard deletion of soft-deleted conversations (MEM-143) and the per-conversation panel (MEM-144) are excluded.
- [MEM-143 — Hard deletion of deleted conversations](docs/increments/active/mem-143-chat-hard-delete/design.md) owns the deployment switch and the worker purge that removes conversations their owner deleted, handing their generated files to the MEM-142 byte sweep; retention windows and deleting uploads are excluded.
- [MEM-144 — Files in this conversation](docs/increments/active/mem-144-session-files/design.md) owns the `sessionId` filter on the file library and the conversation-header panel that lists what a conversation holds.
...
- [MEM-139 — Usage reports](docs/increments/active/mem-139-usage-report/design.md) owns the Onyx usage report export: a period's ZIP of `usage_by_user.csv`, `users.csv` and a Vietnamese PDF review pack, built by the Worker for model managers; VND conversion and message-level export are excluded.
