...
- [Image provider expansion](docs/increments/active/image-provider-expansion/design.md) owns the Azure OpenAI, Google Gemini/Imagen and OpenAI-compatible image adapters, the wider Cloudflare catalog and the redesigned `/admin/image-generation` page; Vertex AI, Bedrock and live-provider acceptance are excluded.
