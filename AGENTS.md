...
- [MEM-135 — Embedding settings](docs/increments/active/mem-135-embedding-settings/design.md) owns choosing the embedding provider and model on an administration page, rebuilding the index in the background from stored chunks while the current one keeps serving, and moving production to self-hosted Qwen3-Embedding-0.6B on the `serving` node GPU; instant switchover and per-Tenant models are excluded. Designed only: no implementation has started.
