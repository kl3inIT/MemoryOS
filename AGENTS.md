...
- [MEM-125 — Query history](docs/increments/active/mem-125-query-history/design.md) owns the administrative read of the Tenant's conversations: a capability of its own, three visibility modes as in Onyx, a list, one transcript, a bounded CSV, and an audit record for every transcript read. Analytics (MEM-174), standard answers (MEM-175) and evals (MEM-176) are excluded.
