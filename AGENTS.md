...
- [Phase 5 — frontend structure, correctness and tooling](docs/increments/active/phase5-frontend/design.md) owns the frontend audit fixes: `@shadcn/lint` and accessibility lint, dead design tokens, the `AppShell` layout route and administration table, route loaders and URL search state, one file preview, the meeting page render split, the generated query factories, shadcn `Field` forms and the shadcn agent skill.
...
- [MEM-195 — Grounded-only answers](docs/increments/active/mem-195-grounded-answers/design.md) owns the Tenant/Agent grounded mode after Amazon Q Business: classify once, forced knowledge search, refusal with the nearest documents when no evidence is selected, text held until the first valid citation, the admin-permitted Web switch, and the topic and phrase guardrails with Group scope; intent routing (MEM-129) and per-claim grounding checks are excluded. Implemented on its branch; staging acceptance and the grounded benchmark baseline remain.
- [Observability dashboards](docs/increments/active/observability-dashboards/design.md) owns the provisioned Service, Chat & AI and Traces dashboards, the Chat turn and guardrail metrics, histogram buckets for the model, Chat and search timers, Tempo span metrics, and turning on telemetry export for production, which had exported nothing since its first promotion; GPU, host and container exporters are excluded.
