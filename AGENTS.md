...
- [MEM-135 — Embedding settings](docs/increments/active/mem-135-embedding-settings/design.md) owns choosing the embedding provider and model on an administration page, rebuilding the index in the background from stored chunks while the current one keeps serving, and moving production to self-hosted Qwen3-Embedding-0.6B on the `serving` node GPU; instant switchover and per-Tenant models are excluded. Search generations, the background rebuild with switch, restore and cleanup, the administration API and the page are implemented; the TEI rollout on `serving` and the production switch remain open.
...
- [Search indexing content errors](docs/increments/active/search-index-content-errors/design.md) owns merging short extraction blocks before chunking, typed indexing error codes on `SourceItem`, the fail-fast coordinator and the per-file error message in the Sources UI.
...
- [OpenAI provider in core](docs/increments/active/openai-provider-in-core/design.md) owns moving the OpenAI Chat provider out of the `api` composition root into `core` under `chat.catalog.openai`. Behaviour, property keys, bean names and the HTTP contract are unchanged.
- [Chat send lock scope](docs/increments/active/chat-send-lock-scope/design.md) owns holding the Chat session lock only through reservation and run registration: opening MCP tools, which may refresh OAuth tokens, runs after it, and a Stop meanwhile cancels the registered run.
- [Library copies use the server-write lifecycle](docs/increments/active/library-copy-server-write/design.md) owns recording a Chat file's stored object on `chat_user_file` (V125), writing library copies and published meeting minutes through `ObjectWriteService`, and removing `ObjectUploadService.write`; browser uploads and the HTTP contract are unchanged.
- [Chat defaults provisioned with the Tenant](docs/increments/active/chat-provision-on-tenant/design.md) owns provisioning Chat's model catalog, flow rows and built-in agent from the `TenantBootstrapped` event in the bootstrap transaction, with the restart re-verification as the backfill, and the read-only catalog reads that follow; the HTTP contract is unchanged.
- [Phase 3 — de-duplication and hot paths](docs/increments/active/phase3-dedup-hot-paths/design.md) owns renaming the integration bundle to `sources/`, one Google Drive/SharePoint sync engine with Onyx failure semantics, the search, Chat-send, SharePoint and meeting hot paths, and the shared helpers.
