...
- [MEM-25 — Audit evidence](docs/increments/active/mem-25-audit/design.md) owns the append-only `audit_event` stream per Tenant, its call sites across IAM, configuration and Sources, the `AUDIT_READ` viewer and retention; shipping to a SIEM is excluded.
