...
- [MEM-122 — Presentation authoring](docs/increments/active/mem-122-deck-authoring/design.md) owns `render-deck` and `check-pptx` in the interpreter executor: a strict deck plan rendered onto one corporate template as `.pptx` or a self-contained HTML deck, with an unfittable slide refused instead of shipped. A Word or Excel template system, Tenant branding and any new Chat tool or endpoint are excluded.
