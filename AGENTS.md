...
- [MEM-91 — Chat voice](docs/increments/active/mem-91-chat-voice/design.md) owns Onyx Voice parity: Tenant voice providers and `/admin/voice`, streaming dictation in Chat and Search, read-aloud, auto-send/auto-playback/auto-listen and user voice settings; audio is never stored. In progress: voice connections, `/admin/voice`, ticketed dictation WebSocket in Chat and Search, Auto-Send, streamed read-aloud with reading speed, Auto-Playback with auto-listen, and ElevenLabs/Azure AI Speech over REST are implemented; text reveal, ElevenLabs realtime and OpenAI Realtime remain open.
...
- [MEM-126 — SharePoint connector](docs/increments/active/sharepoint-connector/design.md) owns the SharePoint Online Source ported from the Onyx connector: Entra app credentials (client secret or certificate), site/library/folder scope, Onyx-style timestamp refresh with scheduled pruning, site pages and the shadcn Sources surfaces. Auto Sync permissions are excluded. Backend is implemented and verified against real PostgreSQL; the Sources screens wait on MEM-106, the access model is still the current `PUBLIC`/`RESTRICTED` contract until MEM-105 merges, and live-tenant acceptance remains open.
