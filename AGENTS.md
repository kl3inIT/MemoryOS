...
- [Test-audit cleanup](docs/increments/active/test-audit-cleanup/design.md) owns removing low-value and duplicated tests and the test-only seams they kept, each contract kept at its strongest existing boundary; `SearchGenerations.fixed`, the cacheless Google Drive metadata cache constructor and the resource measurement harness wait for an owner decision.
