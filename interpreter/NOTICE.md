...
- The executor image adds fonts (Noto core, DejaVu, Liberation, Carlito, Caladea), `LANG=C.UTF-8`, `MPLBACKEND=Agg`, a prebuilt matplotlib cache seeded into `/tmp` by `executor/sitecustomize.py`, `XDG_CACHE_HOME=/tmp/.cache` for fontconfig, poppler-utils, qpdf, sqlite3, unzip/zip, and the Python packages statsmodels, pyarrow, xlrd, chardet, tabulate, jinja2, markdown, beautifulsoup4, markitdown, pdf2image and sympy. It ships `executor/memoryos_charts`, MemoryOS chart extraction started from [e2b-charts](https://pypi.org/project/e2b-charts/) 1.0.0 (MIT, Copyright (c) 2025 FOUNDRYLABS, INC., license kept in `executor/memoryos_charts/LICENSE`), copied as source because the package requires numpy 2 and the executor pins numpy 1.26; `sitecustomize.py` saves figures left open at exit into `.memoryos-charts/`. It also installs `libreoffice-impress-nogui` and the MemoryOS `pptx-to-pdf` helper (`executor/pptx_to_pdf.py`) for the Chat presentation preview, and `libreoffice-calc-nogui` and the MemoryOS `recalc-xlsx` helper (`executor/recalc_xlsx.py`), which recalculates formula values in `.xlsx` workbooks. matplotlib moves from the yanked 3.9.1 to 3.10, and a uv override drops `nvidia-nccl-cu12`.
...
- MemoryOS addition: every `/v1` route requires `X-Api-Key` from `API_KEY_FILE` or `API_KEY`; `/health` stays open. With no key configured the service refuses to start unless `ALLOW_UNAUTHENTICATED=true`, which is how upstream behaves everywhere.
