...
from memoryos_interpreter.services.executor_base import SESSION_NAME_PREFIX, SessionNotFoundError
...
def test_bash_returns_stdout_and_exit_code(executor: DockerExecutor) -> None:
    ...
    with patch("memoryos_interpreter.services.executor_docker.subprocess.Popen", return_value=proc):
...
def test_bash_uses_docker_exec_with_bash_dash_c(executor: DockerExecutor) -> None:
    ...
    with patch(
        "memoryos_interpreter.services.executor_docker.subprocess.Popen", return_value=proc
    ) as popen:
...
def test_bash_times_out_and_kills_bash(executor: DockerExecutor) -> None:
    ...
    with (
        patch("memoryos_interpreter.services.executor_docker.subprocess.Popen", return_value=proc),
        patch("memoryos_interpreter.services.executor_docker.subprocess.run") as run,
    ):
        result = executor.execute_bash_in_session(
            f"{SESSION_NAME_PREFIX}abc",
            cmd="sleep 999",
...
def test_bash_rejects_non_session_id(executor: DockerExecutor) -> None:
    with (
        patch("memoryos_interpreter.services.executor_docker.subprocess.Popen") as popen,
