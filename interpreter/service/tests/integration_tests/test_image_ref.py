"""Tests for ``memoryos_interpreter.image_ref.normalize_image_ref``.
...
def test_bare_repo_gets_latest() -> None:
    assert (
        normalize_image_ref("memoryos-interpreter-executor")
        == "memoryos-interpreter-executor:latest"
    )


def test_namespaced_bare_repo_gets_latest() -> None:
    assert (
        normalize_image_ref("ghcr.io/kl3init/memoryos-interpreter-executor")
        == "ghcr.io/kl3init/memoryos-interpreter-executor:latest"
    )
...
def test_tagged_reference_unchanged() -> None:
    assert (
        normalize_image_ref("memoryos-interpreter-executor:0.4.0")
        == "memoryos-interpreter-executor:0.4.0"
    )
    assert (
        normalize_image_ref("ghcr.io/kl3init/memoryos-interpreter-executor:0.4.0")
        == "ghcr.io/kl3init/memoryos-interpreter-executor:0.4.0"
    )
...
def test_digest_reference_unchanged() -> None:
    digest = (
        "ghcr.io/kl3init/memoryos-interpreter-executor"
...
def test_idempotent_on_already_tagged() -> None:
    ...
    once = normalize_image_ref("ghcr.io/kl3init/memoryos-interpreter-executor")
    twice = normalize_image_ref(once)
    assert once == twice == "ghcr.io/kl3init/memoryos-interpreter-executor:latest"
