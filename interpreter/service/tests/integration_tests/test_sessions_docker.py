...
def test_create_session_returns_session_info(executor: DockerExecutor) -> None:
    before = time.time()
    with patch(
        "memoryos_interpreter.services.executor_docker.subprocess.run", return_value=_completed(0)
    ):
...
def test_create_session_runs_docker_with_session_labels(executor: DockerExecutor) -> None:
    with patch("memoryos_interpreter.services.executor_docker.subprocess.run") as run:
...
def test_create_session_stages_files(executor: DockerExecutor) -> None:
    with (
        patch(
            "memoryos_interpreter.services.executor_docker.subprocess.run",
...
def test_delete_session_rejects_non_session_id(executor: DockerExecutor) -> None:
    ...
    run_mock = MagicMock()
    with patch("memoryos_interpreter.services.executor_docker.subprocess.run", run_mock):
