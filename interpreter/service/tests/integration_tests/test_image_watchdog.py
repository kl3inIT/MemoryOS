...
from memoryos_interpreter.app_configs import PYTHON_EXECUTOR_DOCKER_IMAGE
from memoryos_interpreter.image_ref import normalize_image_ref
from memoryos_interpreter.main import _image_watchdog_once, create_app
...
def _fake_docker(host: FakeDockerHost) -> Iterator[None]:
    with (
        patch("memoryos_interpreter.main.which", return_value="/usr/bin/docker"),
        patch("memoryos_interpreter.main.subprocess.run", side_effect=host.run),
        patch("memoryos_interpreter.services.executor_docker.subprocess.run", side_effect=host.run),
    ):
        yield
...
def test_watchdog_pulls_pinned_digest_not_moving_tag() -> None:
    ...
    pinned = "ghcr.io/kl3init/memoryos-interpreter-executor@sha256:" + "ab" * 32
    host = FakeDockerHost(images=set())

    with (
        patch("memoryos_interpreter.main.PYTHON_EXECUTOR_DOCKER_IMAGE", pinned),
        _fake_docker(host),
    ):
        asyncio.run(_image_watchdog_once())
...
def test_watchdog_pass_returns_when_docker_times_out(timing_out: str) -> None:
    ...
    with (
        patch("memoryos_interpreter.main.which", return_value="/usr/bin/docker"),
        patch("memoryos_interpreter.main.subprocess.run", side_effect=_run),
    ):
        asyncio.run(_image_watchdog_once())  # must not raise
...
def test_lifespan_starts_watchdog_only_for_docker_backend(
    ...
    with (
        patch("memoryos_interpreter.main.EXECUTOR_BACKEND", backend),
        patch(
            "memoryos_interpreter.main.PYTHON_EXECUTOR_DOCKER_IMAGE_WATCHDOG_INTERVAL_SEC",
            interval_sec,
        ),
        patch("memoryos_interpreter.main._ensure_docker_image_available"),
        patch("memoryos_interpreter.main._reap_expired_sessions_once", new=AsyncMock()),
        patch("memoryos_interpreter.main._image_watchdog_loop", new=_fake_loop),
