...
MODULE = "memoryos_interpreter.services.executor_kubernetes"
...
def _deployment(uid: str = "dep-uid-123") -> MagicMock:
    ...
    deployment.metadata.name = "memoryos-interpreter"
...
def test_owner_reference_resolves_to_deployment(executor: KubernetesExecutor) -> None:
    ...
    with (
        patch(f"{MODULE}.KUBERNETES_OWN_NAMESPACE", "memoryos"),
        patch(f"{MODULE}.KUBERNETES_OWNER_DEPLOYMENT_NAME", "memoryos-interpreter"),
    ...
    assert owner.name == "memoryos-interpreter"
...
def test_manifest_carries_owner_reference(executor: KubernetesExecutor) -> None:
    ...
    manifest = executor._create_pod_manifest(
        ...
        command=["sleep", "3600"],
        labels={"app": "memoryos-interpreter", "component": "executor"},
    )
