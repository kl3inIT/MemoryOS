...
from memoryos_interpreter.main import SERVICE_VERSION, create_app
...
CHART_YAML = (
    Path(__file__).resolve().parents[3] / "kubernetes" / "memoryos-interpreter" / "Chart.yaml"
)
...
def test_docker_health_image_missing() -> None:
    ...
    with patch(
        "memoryos_interpreter.services.executor_docker.subprocess.run", side_effect=_side_effect
    ):
        executor = DockerExecutor()
        result = executor.check_health()
