...
def _restore_logging() -> Iterator[None]:
    ...
    Each test reloads the app_configs / logging_config modules under custom env and
    ...
    try:
        ...
        import memoryos_interpreter.app_configs as app_configs
        import memoryos_interpreter.logging_config as logging_config
...
def _reload_logging_config(
    ...
    """Reload the app_configs and logging_config modules under the given env."""
...
def test_plain_logging_output(
    ...
    logging.getLogger("memoryos_interpreter.test").info("hello plain")

    err = capsys.readouterr().err
    assert "hello plain" in err
    assert "memoryos_interpreter.test" in err
...
def test_json_logging_output_and_extra_fields(
    ...
    logging.getLogger("memoryos_interpreter.test").info(
        "hello json", extra={"session_id": "abc123"}
    )
    ...
    assert payload["logger"] == "memoryos_interpreter.test"
