...
from memoryos_interpreter.services.executor_base import StreamChunk, StreamEvent, StreamResult
from memoryos_interpreter.services.executor_kubernetes import KubernetesExecutor
...
def _run_streaming(
    ...
    **kwargs: object,
    ...
    with patch("memoryos_interpreter.services.executor_kubernetes.stream.stream") as mock_stream:
...
def test_stream_exec_uses_fresh_api_client(executor: KubernetesExecutor) -> None:
    with (
        patch(
            "memoryos_interpreter.services.executor_kubernetes.client.ApiClient"
        ) as api_client_cls,
        patch("memoryos_interpreter.services.executor_kubernetes.client.CoreV1Api") as core_v1_cls,
        patch("memoryos_interpreter.services.executor_kubernetes.stream.stream") as mock_stream,
    ):
