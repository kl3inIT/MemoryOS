...
This document provides an in-depth explanation of the memoryos-interpreter service architecture, execution environments, and security model.
...
The memoryos-interpreter service is a secure FastAPI-based platform for executing untrusted Python code in isolated environments. It uses a pluggable executor architecture that supports different isolation backends (Docker, Kubernetes) while maintaining consistent security guarantees.
...
--memory 1024m --memory-swap 1024m  # Hard limit, no swap

# CPU Time (configurable via CPU_TIME_LIMIT_SEC)
--ulimit cpu=30:30  # SIGKILL after 30 seconds of CPU time
...
```bash
docker build -f memoryos-interpreter/Dockerfile .
docker run --privileged memoryos-interpreter
```
...
resources:
  limits:
    memory: "1024Mi"    # Configurable via MEMORY_LIMIT_MB
...
kind: NetworkPolicy
metadata:
  name: memoryos-interpreter-deny-all
spec:
  podSelector:
    matchLabels:
      component: executor
...
| **Memory Bomb** | `--memory 1024m` |
| **CPU Exhaustion** | `--ulimit cpu=30` |
...
- Files stored in `FILE_STORAGE_DIR` (default: `/tmp/memoryos-interpreter-files`)
...
The memoryos-interpreter service provides secure, isolated Python execution through:
