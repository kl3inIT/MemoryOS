...
- Container image for memoryos-interpreter built and available
...
```bash
helm repo add memoryos-interpreter https://onyx-dot-app.github.io/python-sandbox/
helm repo update
```
...
helm install memoryos-interpreter ./kubernetes/memoryos-interpreter
```
...
image:
  repository: my-registry.com/memoryos-interpreter
  tag: v1.0.0
...
codeInterpreter:
  ...
  memoryLimitMb: 512
  kubernetes:
    image: my-registry.com/memoryos-interpreter-executor:v1.0.0

ingress:
  enabled: true
  className: nginx
  hosts:
    - host: memoryos-interpreter.example.com
      paths:
        - path: /
          pathType: Prefix
...
# Install with custom values
helm install memoryos-interpreter ./memoryos-interpreter -f my-values.yaml
```
...
| `image.repository` | Container image repository | `memoryos-interpreter` |
| `image.tag` | Container image tag | `""` (uses chart appVersion) |
| `codeInterpreter.maxExecTimeoutMs` | Maximum execution timeout in milliseconds | `60000` |
| `codeInterpreter.memoryLimitMb` | Memory limit for code execution in MB | `1024` |
| `codeInterpreter.kubernetesExecutor.image` | Container image used for execution pods | `memoryos-interpreter-executor` |
...
```bash
helm install memoryos-interpreter ./memoryos-interpreter \
  --set image.repository=my-registry/memoryos-interpreter \
  --set image.tag=latest \
  --set codeInterpreter.kubernetesExecutor.image=my-registry/memoryos-interpreter-executor:latest
```
...
```bash
helm upgrade my-memoryos-interpreter ./memoryos-interpreter \
...
```bash
helm uninstall my-memoryos-interpreter
```
...
```bash
kubectl get pods -l app.kubernetes.io/name=memoryos-interpreter
...
k port-forward deployment/memoryos-interpreter 8000:8000
...
```bash
# Lint the chart
helm lint ./memoryos-interpreter

# Dry run to see generated manifests
helm install my-memoryos-interpreter ./memoryos-interpreter --dry-run --debug

# Template to generate YAML
helm template my-memoryos-interpreter ./memoryos-interpreter > generated.yaml
```

### Package the chart

```bash
helm package ./memoryos-interpreter
```
